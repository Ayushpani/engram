"""Engram Pipecat service integration.

This module provides a memory service that integrates with Engram to store
...
class EngramPipecatService(FrameProcessor):
    """Memory service that integrates Engram with Pipecat pipelines.

    This service intercepts message frames in the pipeline, retrieves relevant
    memories from Engram, and enhances the context before passing downstream.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the Engram Pipecat service.
        ...
        Args:
            ...
            base_url: Optional custom base URL for Engram API.
        ...
        if not self.api_key:
            raise ConfigurationError(
                "API key is required. Provide api_key parameter or set ENGRAM_API_KEY environment variable."
            )
        ...
        self.params = params or EngramPipecatService.InputParams()
    ...
    async def _retrieve_memories(self, query: str) -> Dict[str, Any]:
        """Retrieve relevant memories from Engram.
        ...
        if self._engram_client is None:
            raise MemoryRetrievalError(
                "Engram client not initialized. Install with: pip install engram"
            )
        ...
        try:
            ...
            response = await self._engram_client.profile(**kwargs)
    ...
    async def _store_messages(self, messages: List[Dict[str, Any]]) -> None:
        """Store messages in Engram (non-blocking, fire-and-forget)."""
        if self._engram_client is None or not messages:
            return
        ...
        try:
            ...
            await self._engram_client.memories.add(**add_params)
    ...
    def _enhance_context_with_memories(
        ...
        Args:
            ...
            memories_data: Memory data from Engram API.
        """
