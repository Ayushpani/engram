"""Engram Cartesia Line agent integration.
...
class EngramCartesiaAgent:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the Engram Cartesia agent wrapper.

        Args:
            agent: The inner Cartesia Line agent to wrap.
            api_key: Engram API key (or ENGRAM_API_KEY env var).
            ...
            base_url: Optional custom Engram API URL.
        ...
        self.config = config or EngramCartesiaAgent.MemoryConfig()

        self.api_key = api_key or os.getenv("ENGRAM_API_KEY")
        if not self.api_key:
            raise ConfigurationError(
                "API key required. Set ENGRAM_API_KEY or pass api_key."
            )
        ...
        self._engram_client = None
        if engram is not None:
            try:
                self._engram_client = engram.AsyncEngram(
                    api_key=self.api_key,
                    base_url=base_url,
                )
                logger.info(f"[Engram] Initialized client for container_tag={container_tag}, all_tags={self.container_tags}")
            except Exception as e:
                logger.error(f"[Engram] Failed to initialize client: {e}")
    ...
    async def _retrieve_memories(self, query: str) -> Dict[str, Any]:
        ...
        if self._engram_client is None:
            raise MemoryRetrievalError("Engram client not initialized")
        ...
        try:
            ...
            logger.info(f"[Engram] Retrieving memories for query: {query[:50]}...")

            response = await asyncio.wait_for(
                self._engram_client.profile(**kwargs),
                timeout=10.0
            )
            ...
            logger.info(f"[Engram] Retrieved memories - static: {static_count}, dynamic: {dynamic_count}, search: {search_count}")
            ...
            logger.warning("[Engram] Profile API timed out after 10s")
            raise MemoryRetrievalError("Profile API timed out")
        except Exception as e:
            logger.error(f"[Engram] Error retrieving memories: {e}")
            raise MemoryRetrievalError("Failed to retrieve memories", e)

    async def _store_messages(self, messages: List[Dict[str, Any]]) -> None:
        """Store messages in Engram."""
        if self._engram_client is None or not messages or self.add_memory == "never":
            return
        ...
        try:
            ...
            logger.info(f"[Engram] Storing {len(messages)} messages to containers={self.container_tags}")
            ...
            logger.info(f"[Engram] Using custom_id={self.custom_id} for document grouping")

            await self._engram_client.add(**add_kwargs)

            logger.info(f"[Engram] Successfully stored {len(messages)} messages")

        except Exception as e:
            logger.error(f"[Engram] Error storing messages: {e}")
    ...
    async def _enrich_event_with_memories(self, event: Any) -> tuple[Any, Optional[str]]:
        ...
        if not user_message:
            logger.warning("[Engram] Could not extract user message from event")
            return event, None
        ...
        logger.info(f"[Engram] Processing user message: {user_message[:50]}...")
        ...
        try:
            ...
            if not memory_context:
                logger.info("[Engram] No memories found for context injection")
                return event, None

            logger.info("[Engram] Retrieved memory context for injection")
            return event, memory_context

        except MemoryRetrievalError as e:
            logger.warning(f"[Engram] Memory retrieval failed: {e}")
            return event, None
        except Exception as e:
            logger.error(f"[Engram] Error in memory enrichment: {e}")
            return event, None
    ...
    async def process(self, env: Any, event: Event) -> AsyncGenerator[Event, None]:
        ...
        try:
            if type(event).__name__ == "UserTurnEnded":
                logger.info("[Engram] Processing UserTurnEnded event")
                ...
                if hasattr(self.agent, 'config'):
                    ...
                    if MEMORY_TAG_START in original_prompt:
                        ...
                        logger.debug("[Engram] Removed old memory context from system prompt")

                    # Inject new memory context if available
                    if memory_context:
                        self.agent.config.system_prompt = f"{memory_context}\n\n{original_prompt}"
                        logger.info("[Engram] Injected new memory context into system prompt")
                    else:
                        # No new memories, but we cleaned up old ones
                        self.agent.config.system_prompt = original_prompt
                        logger.debug("[Engram] No new memories to inject, using clean prompt")
                ...
                if hasattr(event, 'history') and event.history:
                    ...
                    if unsent:
                        logger.info(f"[Engram] Queuing {len(unsent)} messages for storage")
                    ...
                    if user_content:
                        logger.info(f"[Engram] No history, storing current user message: {user_content[:50]}...")
            ...
            logger.error(f"[Engram] Error in process: {e}")
            async for output in self.agent.process(env, event):
                yield output
    ...
    def reset_memory_tracking(self) -> None:
        ...
        logger.info("[Engram] Reset memory tracking state")
