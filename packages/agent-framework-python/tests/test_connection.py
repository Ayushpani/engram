"""Tests for AgentEngram connection class."""
...
from engram_agent_framework import AgentEngram, EngramConfigurationError


class TestAgentEngram:
    def test_requires_api_key(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ENGRAM_API_KEY", None)
            with pytest.raises(EngramConfigurationError):
                AgentEngram()

    def test_accepts_api_key_param(self) -> None:
        conn = AgentEngram(api_key="test-key")
        assert conn.client is not None

    @patch.dict(os.environ, {"ENGRAM_API_KEY": "env-key"})
    ...
    def test_custom_container_tag(self) -> None:
        conn = AgentEngram(api_key="test-key", container_tag="user-123")
    ...
    def test_custom_conversation_id(self) -> None:
        conn = AgentEngram(api_key="test-key", conversation_id="conv-abc")
