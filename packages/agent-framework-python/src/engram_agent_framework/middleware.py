"""Engram middleware for Microsoft Agent Framework.
...
from .connection import AgentEngram
...
@dataclass
class EngramMiddlewareOptions:
    """Configuration options for Engram middleware."""
...
async def _build_memories_text(
    ...
    client: engram.AsyncEngram,
    query_text: str = "",
) -> str:
    """Build formatted memories text from Engram API."""
...
class EngramChatMiddleware(ChatMiddleware):
    """Chat middleware that injects Engram memories into the system prompt.

    This middleware intercepts chat requests before they reach the LLM,
    fetches relevant memories from Engram, and injects them into
    ...
    Example:
        ...
        conn = AgentEngram(api_key="your-key", container_tag="user-123")
    ...
    def __init__(
        self,
        connection: AgentEngram,
        options: Optional[EngramMiddlewareOptions] = None,
    ) -> None:
        self._connection = connection
        self._container_tag = connection.container_tag
        self._options = options or EngramMiddlewareOptions()
        self._logger = create_logger(self._options.verbose)
        self._engram_client = connection.client
