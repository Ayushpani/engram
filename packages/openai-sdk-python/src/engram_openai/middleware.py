"""Engram middleware for OpenAI clients."""
...
class EngramProfileSearch:
    """Type for Engram profile search response."""
...
async def engram_profile_search(
    container_tag: str,
    query_text: str,
    api_key: str,
) -> EngramProfileSearch:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "https://api.engram.ai/v4/profile",
                headers={
                ...
                return EngramProfileSearch(data)
        ...
        return EngramProfileSearch(response.json())
...
async def add_system_prompt(
    ...
    memories_response = await engram_profile_search(
...
async def add_memory_tool(
    ...
    try:
        ...
        # Handle both sync and async engram clients
        ...
        raise EngramMemoryOperationError("Failed to save memory", error)


class EngramOpenAIWrapper:
    """Wrapper for OpenAI client with Engram middleware."""
    ...
    def __init__(
        ...
        if not hasattr(engram, "Engram"):
            raise EngramConfigurationError(
                "engram package is required but not found",
                ImportError("engram package not installed"),
            )

        api_key = self._get_api_key()
        try:
            self._engram_client: engram.Engram = engram.Engram(
                api_key=api_key
            )
        except Exception as e:
            raise EngramConfigurationError(
                f"Failed to initialize Engram client: {e}", e
            )
    ...
    def _get_api_key(self) -> str:
        """Get Engram API key from environment."""
        import os
                ...
                def handle_task_exception(task_obj):
                    try:
                        if task_obj.exception() is not None:
                            ...
                            if isinstance(
                                exception,
                                (EngramNetworkError, EngramAPIError),
                            ):
                                self._logger.warn(
        ...
        **kwargs: Any,
        ...
        if self._options.add_memory == "always":
            ...
            if user_message and user_message.strip():
                ...
                except (EngramAPIError, EngramMemoryOperationError) as e:
...
def with_engram(
    ...
    This middleware searches the engram API for relevant memories using the container tag
    ...
    Example:
        ...
        openai_with_engram = with_engram(
            openai,
        ...
        response = await openai_with_engram.chat.completions.create(
            model="gpt-4",
            messages=[
    ...
    Raises:
        ValueError: When ENGRAM_API_KEY environment variable is not set
        Exception: When engram API request fails
    """
    wrapper = EngramOpenAIWrapper(openai_client, options)
