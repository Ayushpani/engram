"""Engram tools for OpenAI function calling."""
...
from engram.types.search_execute_response import Result
...
class EngramToolsConfig(TypedDict, total=False):
    """Configuration for Engram tools.
...
# Type aliases using inferred types from engram package
...
class EngramTools:
    ...
    def __init__(self, api_key: str, config: Optional[EngramToolsConfig] = None):
        """Initialize EngramTools.

        Args:
            api_key: Engram API key
        ...
        self.client = engram.Engram(**client_kwargs)
...
def create_engram_tools(
    api_key: str, config: Optional[EngramToolsConfig] = None
) -> EngramTools:
    """Helper function to create EngramTools instance.
    ...
    Returns:
        EngramTools instance
    """
    return EngramTools(api_key, config)
...
async def execute_memory_tool_calls(
    ...
    """
    tools = EngramTools(api_key, config)
...
class SearchMemoriesTool:
    ...
    def __init__(self, api_key: str, config: Optional[EngramToolsConfig] = None):
        self.tools = EngramTools(api_key, config)
