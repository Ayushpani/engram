...
@pytest.fixture
def mock_engram_response():
    """Create a mock Engram API response."""
    return {
        "profile": {
            "static": [
...
class TestMiddlewareInitialization:
    ...
    def test_with_engram_basic(self, mock_openai_client):
        """Test basic middleware initialization."""
        with patch.dict(os.environ, {"ENGRAM_API_KEY": "test-key"}):
            options = OpenAIMiddlewareOptions(
                container_tag="user-123",
                custom_id="test-conv"
            )
            wrapped_client = with_engram(mock_openai_client, options)

            assert isinstance(wrapped_client, EngramOpenAIWrapper)
    ...
    def test_with_engram_with_options(self, mock_openai_client):
    ...
    def test_missing_api_key_raises_error(self, mock_openai_client):
        ...
        from engram_openai.exceptions import EngramConfigurationError
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            with pytest.raises(EngramConfigurationError, match="ENGRAM_API_KEY"):
                with_engram(mock_openai_client, options)
...
class TestMemoryInjection:
    ...
    async def test_memory_injection_profile_mode(
        self, mock_async_openai_client, mock_openai_response, mock_engram_response
    ):
        ...
        with patch.dict(os.environ, {"ENGRAM_API_KEY": "test-key"}):
            with patch("engram_openai.middleware.engram_profile_search") as mock_search:
                mock_search.return_value = Mock()
                mock_search.return_value.profile = mock_engram_response["profile"]
                mock_search.return_value.search_results = mock_engram_response["searchResults"]

                wrapped_client = with_engram(
...
class TestMemoryStorage:
    ...
    async def test_add_memory_always_mode(
        ...
        with patch.dict(os.environ, {"ENGRAM_API_KEY": "test-key"}):
            with patch("engram_openai.middleware.engram_profile_search") as mock_search:
                with patch("engram_openai.middleware.add_memory_tool") as mock_add_memory:
                    mock_search.return_value = Mock()
                    mock_search.return_value.profile = {"static": [], "dynamic": []}
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_engram_api_error_handling(
        self, mock_async_openai_client, mock_openai_response
    ):
        """Test handling of Engram API errors."""
        original_create = AsyncMock(return_value=mock_openai_response)
        mock_async_openai_client.chat.completions.create = original_create
...
class TestBackgroundTaskManagement:
    ...
    def test_sync_context_manager_cleanup(
        ...
        with patch.dict(os.environ, {"ENGRAM_API_KEY": "test-key"}):
            with patch("engram_openai.middleware.engram_profile_search") as mock_search:
                with patch("engram_openai.middleware.add_memory_tool"):
                    mock_search.return_value = Mock()
                    mock_search.return_value.profile = {"static": [], "dynamic": []}
                    mock_search.return_value.search_results = {"results": []}
