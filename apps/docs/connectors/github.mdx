...
description: "Connect GitHub repositories to sync documentation files into your Smaran knowledge base"
icon: "github"
---

Connect GitHub repositories to sync documentation files into your Smaran knowledge base with OAuth authentication, webhook support, and automatic incremental syncing.
...
<Tabs>
  ...
  <Tab title="Python">
    ...
    client = Smaran(api_key=os.environ.get("SMARAN_API_KEY"))
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.smaran.ai/v3/connections/github" \
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X GET "https://api.smaran.ai/v3/connections/{connectionId}/resources?page=1&per_page=100" \
      -H "Authorization: Bearer $SMARAN_API_KEY"

    # Configure selected repositories
    curl -X POST "https://api.smaran.ai/v3/connections/{connectionId}/configure" \
      -H "Authorization: Bearer $SMARAN_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "resources": [
...
Smaran provides the API endpoints to list and configure repositories. As a Smaran customer, you need to build the UI in your application where your end-users can:
...
Files are indexed as `github_markdown` document type in Smaran.
...
The GitHub connector automatically sets up webhooks for real-time incremental syncing. When files are pushed or deleted in configured repositories, Smaran is notified immediately.

<Note>
**Batch Processing:** Webhook events are processed in batches with a 10-minute delay to optimize performance and prevent excessive syncing during rapid commits. This means changes pushed to your repository will be reflected in Smaran within approximately 10 minutes.
</Note>
...
3. **Delete Events**: When documentation files are deleted, they're removed from your Smaran knowledge base
4. **Incremental Updates**: Only changed files are processed, keeping sync fast and efficient

### Webhook Security

Webhooks are secured using HMAC-SHA256 signature validation with constant-time comparison. Smaran automatically validates that webhook events come from GitHub before processing them. Each repository gets a unique webhook secret for maximum security.
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "https://api.smaran.ai/v3/connections/list" \
      -H "Authorization: Bearer $SMARAN_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{}'
    ```
...
- Existing documents from removed repositories remain in Smaran unless you delete them manually
</Note>
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    # Delete by connection ID
    curl -X DELETE "https://api.smaran.ai/v3/connections/{connectionId}" \
      -H "Authorization: Bearer $SMARAN_API_KEY"
    ```
  </Tab>
</Tabs>
...
- **Permanently delete all synced documents** from your Smaran knowledge base (unless you pass `deleteDocuments=false` as a query parameter to keep them)
</Warning>
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "https://api.smaran.ai/v3/connections/github/import" \
      -H "Authorization: Bearer $SMARAN_API_KEY"
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    # Get current settings
    curl -X GET "https://api.smaran.ai/v3/settings" \
      -H "Authorization: Bearer $SMARAN_API_KEY"
    ```
  </Tab>
</Tabs>
...
3. Set Authorization callback URL to: `https://api.smaran.ai/v3/connections/auth/callback/github`
4. Copy the Client ID and generate a Client Secret
5. Configure them in Smaran using the settings API above

After configuration, all new GitHub connections will use your custom OAuth app instead of Smaran's default app.
</Note>
