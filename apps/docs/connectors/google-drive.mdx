...
description: "Connect Google Drive to sync documents into your Engram knowledge base"
icon: "google-drive"
---

Connect Google Drive to sync documents into your Engram knowledge base with OAuth authentication and custom app support.

## Sync scope

**Default for new connections:** after OAuth, the user completes a **folder and file** picker (Google Docs, Sheets, Slides, and PDFs). Only items they select are synced and updated until they change the selection (for example from the Engram console).
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.engram.ai/v3/connections/google-drive" \
...
After the user grants permissions, Google redirects through Engram to finish the connection. With **scoped** sync (`syncScope` omitted or `"selected"`), the user is sent to Engram’s **hosted file and folder picker**; they must complete that step before imports run. With **`syncScope: "full"`**, Engram redirects to your `redirectUrl` (or returns connection details) **without** the picker. You can open the picker again later for an existing connection (Engram console, or `POST /v3/connections/{connectionId}/google-drive/hosted-picker` with an authenticated admin session).
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    # Delete by connection ID
    curl -X DELETE "https://api.engram.ai/v3/connections/connection_id_123" \
      -H "Authorization: Bearer $ENGRAM_API_KEY"

    # Response: {"id": "connection_id_123", "provider": "google-drive"}

    # Delete by provider and container tags
    curl -X DELETE "https://api.engram.ai/v3/connections/google-drive" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "containerTags": ["user-123"]
...
- Keep existing synced documents in Engram (they won't be deleted)
</Note>
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "https://api.engram.ai/v3/connections/google-drive/import" \
      -H "Authorization: Bearer $ENGRAM_API_KEY"
