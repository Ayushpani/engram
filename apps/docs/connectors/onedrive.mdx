...
Connect OneDrive to automatically sync Word documents, Excel spreadsheets, and PowerPoint presentations into your Smaran knowledge base. Supports both personal and business accounts with scheduled synchronization.
...
```bash cURL
curl -X POST "https://api.smaran.ai/v3/connections/onedrive" \
...
<CodeGroup>
  ...
  ```bash cURL
    # Trigger manual sync
    curl -X POST "https://api.smaran.ai/v3/connections/onedrive/import" \
      -H "Authorization: Bearer $SMARAN_API_KEY" \
      -H "Content-Type: application/json" \
...
# Delete by provider and container tags
curl -X DELETE "https://api.smaran.ai/v3/connections/onedrive" \
  -H "Authorization: Bearer $SMARAN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"containerTags": ["user-123"]}'
```
