...
Connect Gmail to automatically sync email threads into your engram knowledge base. Supports real-time updates via Google Cloud Pub/Sub webhooks and incremental synchronization.
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.engram.ai/v3/connections/gmail" \
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    # Delete by provider and container tags
    curl -X DELETE "https://api.engram.ai/v3/connections/gmail" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "containerTags": ["user-123"]
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "https://api.engram.ai/v3/connections/gmail/import" \
      -H "Authorization: Bearer $ENGRAM_API_KEY"
...
1. When a connection is created, engram registers a Gmail API "watch" subscription
2. Gmail sends notifications to a Google Cloud Pub/Sub topic when emails change
3. engram receives these notifications and fetches updated threads
...
- **Watch expiration**: Gmail watch subscriptions expire after 7 days (automatically renewed by engram)
...
- Real-time sync requires engram's Pub/Sub infrastructure
