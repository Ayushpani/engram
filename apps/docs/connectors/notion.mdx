...
icon: "notion"
---
Connect Notion workspaces to automatically sync pages, databases, and content blocks into your Engram knowledge base. Supports real-time updates, rich formatting, and database properties.
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.engram.ai/v3/connections/notion" \
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "https://api.engram.ai/v3/connections/notion/connection" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
    ...
    curl -X POST "https://api.engram.ai/v3/connections/notion/documents" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"containerTags": ["user-123", "notion-workspace"]}'
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    # Delete by provider and container tags
    curl -X DELETE "https://api.engram.ai/v3/connections/notion" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
