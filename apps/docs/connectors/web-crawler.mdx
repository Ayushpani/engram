...
Connect websites to automatically crawl and sync web pages into your Engram knowledge base. The web crawler respects robots.txt rules, includes SSRF protection, and automatically recrawls sites on a schedule.
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.engram.ai/v3/connections/web-crawler" \
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    curl -X POST "https://api.engram.ai/v3/connections/web-crawler/connection" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
    ...
    curl -X POST "https://api.engram.ai/v3/connections/web-crawler/documents" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"containerTags": ["user-123", "website-sync"]}'
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    # Delete by provider and container tags
    curl -X DELETE "https://api.engram.ai/v3/connections/web-crawler" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"containerTags": ["user-123"]}'
    ```
