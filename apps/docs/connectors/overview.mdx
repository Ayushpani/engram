...
Connect external platforms to automatically sync documents into engram. Supported connectors include Google Drive, Gmail, Notion, OneDrive, GitHub and Web Crawler with real-time synchronization and intelligent content processing.
...
```bash cURL
# Delete connection and all imported documents (default)
curl -X DELETE "https://api.engram.ai/v3/connections/conn_abc123" \
  -H "Authorization: Bearer $ENGRAM_API_KEY"

# Delete connection but keep imported documents
curl -X DELETE "https://api.engram.ai/v3/connections/conn_abc123?deleteDocuments=false" \
  -H "Authorization: Bearer $ENGRAM_API_KEY"
