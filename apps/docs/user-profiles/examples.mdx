...
async function handleChatMessage(userId: string, message: string) {
  // Get user profile
  const profileResponse = await fetch('https://api.smaran.ai/v4/profile', {
    method: 'POST',
    headers: {
...
async def handle_chat_message(user_id: str, message: str):
    ...
    response = requests.post(
        'https://api.smaran.ai/v4/profile',
        headers={
            'Authorization': f'Bearer {os.getenv("SMARAN_API_KEY")}',
            'Content-Type': 'application/json'
        },
...
async function getFullContext(userId: string, userQuery: string) {
  ...
  const response = await fetch('https://api.smaran.ai/v4/profile', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${process.env.SMARAN_API_KEY}`,
      'Content-Type': 'application/json'
...
async function advancedContext(userId: string, query: string) {
  ...
  const [profileRes, searchRes] = await Promise.all([
    fetch('https://api.smaran.ai/v4/profile', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${process.env.SMARAN_API_KEY}`,
        'Content-Type': 'application/json'
    ...
    fetch('https://api.smaran.ai/v3/search', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${process.env.SMARAN_API_KEY}`,
        'Content-Type': 'application/json'
...
For the cleanest integration, use the Smaran AI SDK middleware:
