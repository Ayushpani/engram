...
<Tabs>
  <Tab title="fetch">
    ```typescript
    const response = await fetch("https://api.engram.ai/v4/memories", {
      method: "POST",
      headers: {
...
Use **Create Memories** when you already know the exact facts to store (user preferences, traits, structured data). Use **Add Context** when you have raw content (conversations, documents, URLs) that Engram should process and extract memories from.
</Tip>
...
<Tabs>
  <Tab title="fetch">
    ```typescript
    await fetch("https://api.engram.ai/v4/memories/mem_abc123/forget", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${API_KEY}`
      }
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.engram.ai/v4/memories/mem_abc123/forget" \
      -H "Authorization: Bearer $ENGRAM_API_KEY"
    ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="fetch">
    ```typescript
    await fetch("https://api.engram.ai/v4/memories", {
      method: "PATCH",
      headers: {
        "Authorization": `Bearer ${API_KEY}`,
        "Content-Type": "application/json"
  ...
  <Tab title="cURL">
    ```bash
    curl -X PATCH "https://api.engram.ai/v4/memories" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "id": "mem_abc123",
