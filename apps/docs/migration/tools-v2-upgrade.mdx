---
title: 'Upgrading @engram/tools to v2.0.0'
description: 'Migrate your code from @engram/tools 1.4.x to 2.0.0 — config-object signature, customId, and new defaults'
sidebarTitle: 'Tools: v1.4 → 2.0'
---

`@engram/tools` v2.0.0 unifies the API across all four integrations (Vercel AI SDK, OpenAI, Mastra, VoltAgent) around a single config-object signature and a consistent conversation-grouping concept. This guide walks you through the breaking changes.
...
| Area                  | v1.4.x                                                | v2.0.0                                                      |
| --------------------- | ----------------------------------------------------- | ----------------------------------------------------------- |
| Signature             | `withEngram(model, "user-123", { ... })`         | `withEngram(model, { containerTag: "user-123", ... })` |
...
```bash
npm install @engram/tools@^2.0.0
```
...
const model = withEngram(openai('gpt-4'), 'user-123', {
...
const client = withEngram(openai, 'user-123', {
  conversationId: 'conv-456',
});
```
...
const client = withEngram(openai, {
  containerTag: 'user-123',
  customId: 'conv-456',
});
```
...
} from '@engram/tools/mastra';

const input = new EngramInputProcessor('user-123', {
  mode: 'full',
});

const output = createEngramOutputProcessor('user-123', {
  threadId: 'conv-456',
  addMemory: 'always',
});
```
...
const input = new EngramInputProcessor({
  containerTag: 'user-123',
  customId: 'conv-456',
  mode: 'full',
});

const output = createEngramOutputProcessor({
  containerTag: 'user-123',
  customId: 'conv-456',
});
```
...
<Steps>
  ...
  <Step title="Find every withEngram / processor call">
    Grep your codebase for `withEngram(`, `EngramInputProcessor`,
    `EngramOutputProcessor`, `createEngramProcessor`,
    `createEngramOutputProcessor`.
  </Step>
...
If you hit something this guide does not cover, open an issue on [GitHub](https://github.com/engramai/engram).
