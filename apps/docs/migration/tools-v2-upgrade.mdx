---
title: 'Upgrading @smaranai/tools to v2.0.0'
description: 'Migrate your code from @smaranai/tools 1.4.x to 2.0.0 — config-object signature, customId, and new defaults'
sidebarTitle: 'Tools: v1.4 → 2.0'
---

`@smaranai/tools` v2.0.0 unifies the API across all four integrations (Vercel AI SDK, OpenAI, Mastra, VoltAgent) around a single config-object signature and a consistent conversation-grouping concept. This guide walks you through the breaking changes.
...
| Area                  | v1.4.x                                                | v2.0.0                                                      |
| --------------------- | ----------------------------------------------------- | ----------------------------------------------------------- |
| Signature             | `withSmaran(model, "user-123", { ... })`         | `withSmaran(model, { containerTag: "user-123", ... })` |
...
```bash
npm install @smaranai/tools@^2.0.0
```
...
const model = withSmaran(openai('gpt-4'), 'user-123', {
...
const client = withSmaran(openai, 'user-123', {
  conversationId: 'conv-456',
});
```
...
} from '@smaranai/tools/mastra';

const input = new SmaranInputProcessor('user-123', {
  mode: 'full',
});

const output = createSmaranOutputProcessor('user-123', {
  threadId: 'conv-456',
  addMemory: 'always',
});
```
...
const input = new SmaranInputProcessor({
  containerTag: 'user-123',
  customId: 'conv-456',
  mode: 'full',
});

const output = createSmaranOutputProcessor({
  containerTag: 'user-123',
  customId: 'conv-456',
});
```
...
<Steps>
  ...
  <Step title="Find every withSmaran / processor call">
    Grep your codebase for `withSmaran(`, `SmaranInputProcessor`,
    `SmaranOutputProcessor`, `createSmaranProcessor`,
    `createSmaranOutputProcessor`.
  </Step>
...
If you hit something this guide does not cover, open an issue on [GitHub](https://github.com/smaranai/smaran).
