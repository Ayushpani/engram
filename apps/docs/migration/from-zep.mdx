---
title: "Migrating from Zep to Smaran"
description: "Quick guide to migrate from Zep to Smaran"
...
client = Smaran(api_key="your-api-key")
```

```typescript TypeScript
import { Smaran } from "smaran";
const client = new Smaran({ apiKey: "your-api-key" });
```
...
1. **Replace client initialization** - Use Smaran client instead of Zep
...
- **Response structure** - Smaran returns chunks with scores, not direct memory content
...
const smaran = new Smaran({ apiKey: "your_smaran_api_key" });

// Export from Zep and import to Smaran
...
async function migrateFromZep(
  ...
  const smaran = new Smaran({ apiKey: smaranApiKey });
...
- [Smaran SDKs](/integrations/smaran-sdk)
