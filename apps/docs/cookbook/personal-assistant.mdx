...
<CardGroup cols={2}>
  ...
  <Card title="TypeScript + AI SDK" icon="triangle" href="#typescript-implementation">
    Modern React approach. Uses Next.js + Vercel AI SDK + Smaran tools.
  </Card>
</CardGroup>
...
- **Smaran API key** ([get one here](https://console.smaran.ai))
...
pip install smaran openai fastapi uvicorn python-dotenv streamlit requests
```

Create a `.env` file:

```bash
SMARAN_API_KEY=your_smaran_key_here
...
- **Smaran**: Client for storing and retrieving long-term memories
...
smaran_client = Smaran(api_key=os.getenv("SMARAN_API_KEY"))
```
...
- **Smaran client**: Stores and retrieves user-specific memories
...
async def search_user_memories(query: str, container_tag: str) -> str:
    try:
        results = smaran_client.search.memories(
            q=query,
...
cd personal-ai
npm install @smaranai/tools ai @ai-sdk/openai
```
...
- **smaranTools**: Pre-built tools for memory search and storage
...
4. Stores in Smaran with the user's container tag
...
load_dotenv()
client = Smaran(api_key=os.getenv("SMARAN_API_KEY"))
...
const response = await fetch('https://api.smaran.ai/v3/memories', {
  method: 'POST',
  headers: {
...
*Built with Smaran. Customize based on your needs.*
