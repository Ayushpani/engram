...
<CardGroup cols={2}>
  ...
  <Card title="TypeScript + AI SDK" icon="triangle" href="#typescript-implementation">
    Modern React approach. Uses Next.js + Vercel AI SDK + Engram tools.
  </Card>
</CardGroup>
...
- **Engram API key** ([get one here](https://console.engram.ai))
...
pip install engram openai fastapi uvicorn python-dotenv streamlit requests
```

Create a `.env` file:

```bash
ENGRAM_API_KEY=your_engram_key_here
...
- **Engram**: Client for storing and retrieving long-term memories
...
engram_client = Engram(api_key=os.getenv("ENGRAM_API_KEY"))
```
...
- **Engram client**: Stores and retrieves user-specific memories
...
async def search_user_memories(query: str, container_tag: str) -> str:
    try:
        results = engram_client.search.memories(
            q=query,
...
cd personal-ai
npm install @engram/tools ai @ai-sdk/openai
```
...
- **engramTools**: Pre-built tools for memory search and storage
...
4. Stores in Engram with the user's container tag
...
load_dotenv()
client = Engram(api_key=os.getenv("ENGRAM_API_KEY"))
...
const response = await fetch('https://api.engram.ai/v3/memories', {
  method: 'POST',
  headers: {
...
*Built with Engram. Customize based on your needs.*
