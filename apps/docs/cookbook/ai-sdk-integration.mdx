...
description: "Complete examples showing how to use Smaran with Vercel AI SDK for building intelligent applications"
---

This page provides comprehensive examples of using Smaran with the Vercel AI SDK, covering Memory Tools and User Profiles approaches.
...
import { addMemory } from '@smaranai/tools'
...
export async function POST(request: Request) {
  ...
  const memory = await addMemory({
    ...
    content: file, // Smaran handles file processing
    title: file.name,
    headers: {
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    tools: {
      ...
      searchMemories: searchMemoriesTool(process.env.SMARAN_API_KEY!, {
        headers: {
        }
      }),
      addMemory: addMemoryTool(process.env.SMARAN_API_KEY!, {
        headers: {
        }
      }),
      // Add custom tools
...
Combine Smaran tools with your own custom tools:
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    tools: {
      // Spread Smaran tools
      ...smaranTools(process.env.SMARAN_API_KEY!),
      // Add custom tools
