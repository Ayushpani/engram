...
description: "Complete examples showing how to use Engram with Vercel AI SDK for building intelligent applications"
---

This page provides comprehensive examples of using Engram with the Vercel AI SDK, covering Memory Tools and User Profiles approaches.
...
import { addMemory } from '@engram/tools'
...
export async function POST(request: Request) {
  ...
  const memory = await addMemory({
    ...
    content: file, // Engram handles file processing
    title: file.name,
    headers: {
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    tools: {
      ...
      searchMemories: searchMemoriesTool(process.env.ENGRAM_API_KEY!, {
        headers: {
        }
      }),
      addMemory: addMemoryTool(process.env.ENGRAM_API_KEY!, {
        headers: {
        }
      }),
      // Add custom tools
...
Combine Engram tools with your own custom tools:
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    tools: {
      // Spread Engram tools
      ...engramTools(process.env.ENGRAM_API_KEY!),
      // Add custom tools
