...
| Option | Description |
|--------|-------------|
| `-p, --provider` | Memory provider (`engram`, `mem0`, `zep`) |
...
```bash
bun run src/index.ts compare -p engram,mem0,zep -b locomo -j gpt-4o
```
...
```bash
bun run src/index.ts run -p engram -b longmemeval -s 3 --sample-type random
```
