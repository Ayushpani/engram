...
| Option | Description |
|--------|-------------|
| `-p, --provider` | Memory provider (`smaran`, `mem0`, `zep`) |
...
```bash
bun run src/index.ts compare -p smaran,mem0,zep -b locomo -j gpt-4o
```
...
```bash
bun run src/index.ts run -p smaran -b longmemeval -s 3 --sample-type random
```
