---
title: "engram Infinite Chat"
description: "Build chat applications with unlimited context using engram's intelligent proxy"
tag: "BETA"
---

import GettingAPIKey from '/snippets/getting-api-key.mdx';

engram Infinite Chat is a powerful solution that gives your chat applications unlimited contextual memory. It works as a transparent proxy in front of your existing LLM provider, intelligently managing long conversations without requiring any changes to your application logic.
...
### 1. Get a engram API key

<GettingAPIKey />

### 2. Add engram in front of any **OpenAI-Compatible** API URL
...
/**
 * Initialize the OpenAI client with engram proxy
 * @param {string} OPENAI_API_KEY - Your OpenAI API key
 * @param {string} ENGRAM_API_KEY - Your engram API key
...
# Configure the OpenAI client with engram proxy
openai.api_base = "https://api.engram.ai/v3/https://api.openai.com/v1"
openai.api_key = os.environ.get("OPENAI_API_KEY")  # Your regular OpenAI key
openai.default_headers = {
    "x-engram--api-key": os.environ.get("ENGRAM_API_KEY"),  # Your engram key
}
...
<Steps>
  <Step title="Transparent Proxying">
    All requests pass through engram to your chosen LLM provider with zero latency overhead.
  ...
  <Step title="Smart Retrieval">
    When conversations exceed token limits (20k+), engram intelligently retrieves the most relevant context from previous messages.
  </Step>
...
<Accordion title="Unlimited Context" icon="infinity">
  No more 8k/32k/128k token limits - conversations can extend indefinitely with engram's advanced retrieval system.
</Accordion>
...
<Note>
  engram is designed with reliability as the top priority. If any issues occur within the engram processing pipeline, the system will automatically fall back to direct forwarding of your request to the LLM provider, ensuring zero downtime for your applications.
</Note>
...
| Header                           | Description                                                            |
| -------------------------------- | ---------------------------------------------------------------------- |
| `x-engram-conversation-id`  | Unique identifier for the conversation thread                          |
| `x-engram-context-modified` | Indicates whether engram modified the context ("true" or "false") |
| `x-engram-tokens-processed` | Number of tokens processed in this request                             |
| `x-engram-chunks-created`   | Number of new chunks created from this conversation                    |
| `x-engram-chunks-deleted`   | Number of chunks removed (if any)                                      |
| `x-engram-docs-deleted`     | Number of documents removed (if any)                                   |

If an error occurs, an additional header `x-engram-error` will be included with details about what went wrong. Your request will still be processed by the underlying LLM provider even if engram encounters an error.

## Rate Limiting

<Info>
  Currently, there are no rate limits specific to engram. Your requests are subject only to the rate limits of your underlying LLM provider.
</Info>

## Supported Models

engram works with any OpenAI-compatible API, including:
