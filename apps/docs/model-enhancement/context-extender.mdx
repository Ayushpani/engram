---
title: "smaran Infinite Chat"
description: "Build chat applications with unlimited context using smaran's intelligent proxy"
tag: "BETA"
---

import GettingAPIKey from '/snippets/getting-api-key.mdx';

smaran Infinite Chat is a powerful solution that gives your chat applications unlimited contextual memory. It works as a transparent proxy in front of your existing LLM provider, intelligently managing long conversations without requiring any changes to your application logic.
...
### 1. Get a smaran API key

<GettingAPIKey />

### 2. Add smaran in front of any **OpenAI-Compatible** API URL
...
/**
 * Initialize the OpenAI client with smaran proxy
 * @param {string} OPENAI_API_KEY - Your OpenAI API key
 * @param {string} SMARAN_API_KEY - Your smaran API key
...
const client = new OpenAI({
  ...
  headers: {
    "x-smaran-api-key": process.env.SMARAN_API_KEY,
...
# Configure the OpenAI client with smaran proxy
openai.api_base = "https://api.smaran.ai/v3/https://api.openai.com/v1"
openai.api_key = os.environ.get("OPENAI_API_KEY")  # Your regular OpenAI key
openai.default_headers = {
    "x-smaran--api-key": os.environ.get("SMARAN_API_KEY"),  # Your smaran key
}
...
<Steps>
  <Step title="Transparent Proxying">
    All requests pass through smaran to your chosen LLM provider with zero latency overhead.
  ...
  <Step title="Smart Retrieval">
    When conversations exceed token limits (20k+), smaran intelligently retrieves the most relevant context from previous messages.
  </Step>
...
<Accordion title="Unlimited Context" icon="infinity">
  No more 8k/32k/128k token limits - conversations can extend indefinitely with smaran's advanced retrieval system.
</Accordion>
...
<Note>
  smaran is designed with reliability as the top priority. If any issues occur within the smaran processing pipeline, the system will automatically fall back to direct forwarding of your request to the LLM provider, ensuring zero downtime for your applications.
</Note>
...
| Header                           | Description                                                            |
| -------------------------------- | ---------------------------------------------------------------------- |
| `x-smaran-conversation-id`  | Unique identifier for the conversation thread                          |
| `x-smaran-context-modified` | Indicates whether smaran modified the context ("true" or "false") |
| `x-smaran-tokens-processed` | Number of tokens processed in this request                             |
| `x-smaran-chunks-created`   | Number of new chunks created from this conversation                    |
| `x-smaran-chunks-deleted`   | Number of chunks removed (if any)                                      |
| `x-smaran-docs-deleted`     | Number of documents removed (if any)                                   |

If an error occurs, an additional header `x-smaran-error` will be included with details about what went wrong. Your request will still be processed by the underlying LLM provider even if smaran encounters an error.

## Rate Limiting

<Info>
  Currently, there are no rate limits specific to smaran. Your requests are subject only to the rate limits of your underlying LLM provider.
</Info>

## Supported Models

smaran works with any OpenAI-compatible API, including:
