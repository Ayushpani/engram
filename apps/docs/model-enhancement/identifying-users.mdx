...
description: "Identifying users in engram"
---

You can enable built-in cross-conversational memory by sending engram a `x-sm-user-id`.

## How engram Identifies Users and conversations

engram will find the user ID in the following places (in order of priority):
...
If a conversation identifier is provided, You do not need to send the entire array of messages to engram.

```typescript
// if you provide conversation ID, You do not need to send all the messages every single time. engram automatically backfills it.
