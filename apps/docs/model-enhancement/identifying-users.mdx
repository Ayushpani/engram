...
description: "Identifying users in smaran"
---

You can enable built-in cross-conversational memory by sending smaran a `x-sm-user-id`.

## How smaran Identifies Users and conversations

smaran will find the user ID in the following places (in order of priority):
...
If a conversation identifier is provided, You do not need to send the entire array of messages to smaran.

```typescript
// if you provide conversation ID, You do not need to send all the messages every single time. smaran automatically backfills it.
