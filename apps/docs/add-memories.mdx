---
title: "Ingesting context to smaran"
sidebarTitle: "Add context"
description: "Add text, files, and URLs to Smaran"
icon: "plus"
---

Send any raw content to Smaran — conversations, documents, files, URLs. We extract the memories automatically.
...
Use `customId` to update existing documents or conversations. When you send content with the same `customId`, Smaran intelligently processes only what's new.
...
await client.add({
  ...
  customId: "conv_123",  // Same ID — Smaran links them
  containerTag: "user_sarah"
});
```
...
```typescript
// Smaran detects the diff and only processes new parts
await client.add({
...
Format your conversations however you want. Smaran handles any string format:
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.smaran.ai/v3/documents/file" \
      -H "Authorization: Bearer $SMARAN_API_KEY" \
      -F "file=@document.pdf" \
...
<AccordionGroup>
  <Accordion title="Parameter Details & Examples">
    ...
    **Custom IDs (Recommended):**
    ...
    // Smaran only processes new/changed content
    await client.add({
...
By default, when you add content, Smaran uses **all** existing memories in the space as context for generating new memories. With **filtered writes**, you can scope this context to only memories from documents matching specific metadata.
...
<AccordionGroup>
  ...
  <Accordion title="Error Handling">
    ...
    ```typescript
    import { BadRequestError, RateLimitError } from 'smaran';
