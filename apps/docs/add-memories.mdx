---
title: "Ingesting context to engram"
sidebarTitle: "Add context"
description: "Add text, files, and URLs to Engram"
icon: "plus"
---

Send any raw content to Engram — conversations, documents, files, URLs. We extract the memories automatically.
...
Use `customId` to update existing documents or conversations. When you send content with the same `customId`, Engram intelligently processes only what's new.
...
await client.add({
  ...
  customId: "conv_123",  // Same ID — Engram links them
  containerTag: "user_sarah"
});
```
...
```typescript
// Engram detects the diff and only processes new parts
await client.add({
...
Format your conversations however you want. Engram handles any string format:
...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.engram.ai/v3/documents/file" \
      -H "Authorization: Bearer $ENGRAM_API_KEY" \
      -F "file=@document.pdf" \
...
<AccordionGroup>
  <Accordion title="Parameter Details & Examples">
    ...
    **Custom IDs (Recommended):**
    ...
    // Engram only processes new/changed content
    await client.add({
...
By default, when you add content, Engram uses **all** existing memories in the space as context for generating new memories. With **filtered writes**, you can scope this context to only memories from documents matching specific metadata.
...
<AccordionGroup>
  ...
  <Accordion title="Error Handling">
    ...
    ```typescript
    import { BadRequestError, RateLimitError } from 'engram';
