...
Before searching memories, you need to set up the Smaran client:
...
- **Memories:** They are automatically extracted from your documents by Smaran. Smaller information chunks inferred from documents and related to each other.
...
```bash cURL
curl -X GET "https://api.smaran.ai/v3/documents/{YOUR-DOCUMENT-ID}" \
  -H "Authorization: Bearer $SMARAN_API_KEY"
```
...
By default, Smaran returns chunks **with context** (surrounding text):
