...
Before searching memories, you need to set up the Engram client:
...
- **Memories:** They are automatically extracted from your documents by Engram. Smaller information chunks inferred from documents and related to each other.
...
```bash cURL
curl -X GET "https://api.engram.ai/v3/documents/{YOUR-DOCUMENT-ID}" \
  -H "Authorization: Bearer $ENGRAM_API_KEY"
```
...
By default, Engram returns chunks **with context** (surrounding text):
