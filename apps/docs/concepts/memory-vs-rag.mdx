...
## Documents vs Memories in Smaran

Smaran makes a clear distinction between these two concepts:

### Documents: Raw Knowledge
Documents are the raw content you send to Smaran—PDFs, web pages, text files. They represent static knowledge that doesn't change based on who's accessing it.
...
<Tabs>
  ...
  <Tab title="Memory Approach (Right)">
    ```python
    # Smaran understands temporal context
    query = "What sneakers should I buy?"
...
## How Smaran Handles Both

Smaran provides a unified platform that correctly handles both patterns:
...
Smaran provides both capabilities in a unified platform, ensuring your agents have the right context at the right time.
