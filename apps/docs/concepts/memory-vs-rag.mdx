...
## Documents vs Memories in Engram

Engram makes a clear distinction between these two concepts:

### Documents: Raw Knowledge
Documents are the raw content you send to Engram—PDFs, web pages, text files. They represent static knowledge that doesn't change based on who's accessing it.
...
<Tabs>
  ...
  <Tab title="Memory Approach (Right)">
    ```python
    # Engram understands temporal context
    query = "What sneakers should I buy?"
...
## How Engram Handles Both

Engram provides a unified platform that correctly handles both patterns:
...
Engram provides both capabilities in a unified platform, ensuring your agents have the right context at the right time.
