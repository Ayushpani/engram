...
Smaran builds a living knowledge graph where memories connect to other memories. Unlike traditional knowledge graphs with entity-relation-entity triples, Smaran's graph is **facts built on top of other facts**.

## Memory Relationships

When you add content, Smaran extracts facts and automatically connects them to existing memories through three relationship types:
...
When Smaran infers new facts from patterns:
...
From a single conversation, Smaran extracts multiple connected memories:
...
Smaran knows when memories become irrelevant:
...
Smaran distinguishes memory types automatically:
