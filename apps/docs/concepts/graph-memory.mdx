...
Engram builds a living knowledge graph where memories connect to other memories. Unlike traditional knowledge graphs with entity-relation-entity triples, Engram's graph is **facts built on top of other facts**.

## Memory Relationships

When you add content, Engram extracts facts and automatically connects them to existing memories through three relationship types:
...
When Engram infers new facts from patterns:
...
From a single conversation, Engram extracts multiple connected memories:
...
Engram knows when memories become irrelevant:
...
Engram distinguishes memory types automatically:
