...
description: "Engram provides a managed RAG solution - extraction, indexing, storing, and retrieval."
icon: "bolt"
---

Engram doesn't just store your content—it transforms it into optimized, searchable knowledge. Every upload goes through an intelligent pipeline that extracts, chunks, and indexes content in the ideal way for its type.
...
```typescript
// Just add content — Engram handles the rest
await client.add({
  content: pdfBase64,
  contentType: "pdf",
...
Different content types need different chunking strategies. Engram applies the optimal approach automatically:
...
Code is chunked using [code-chunk](https://github.com/engramai/code-chunk), our open-source library that understands AST (Abstract Syntax Tree) boundaries:
...
Engram combines the best of both approaches in every search:
...
You focus on building your product. Engram handles the RAG complexity.
