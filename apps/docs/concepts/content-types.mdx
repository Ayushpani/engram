...
description: "All the content formats Engram can ingest and process"
icon: "file-stack"
---

Engram automatically extracts and indexes content from various formats. Just send it—we handle the rest. See [Add Memories](/add-memories) to learn how to ingest content via the API.
...
Send a URL and Engram fetches, extracts, and indexes the content.
...
Code is chunked using [code-chunk](https://github.com/engramai/code-chunk), which understands AST boundaries to keep functions, classes, and logical blocks intact. See [Super RAG](/concepts/super-rag) for how Engram optimizes chunking for each content type.
...
If you don't specify `contentType`, Engram auto-detects:
