...
description: "Automatic Smaran integration using AI coding agents"
icon: "zap"
sidebarTitle: "Install with AI"
---

Get your AI coding agent to integrate Smaran in minutes. Copy the prompt below, paste it into Claude/GPT/Cursor, and let it do the work.

## Quick Setup

<CardGroup cols={3}>
	<Card title="MCP (Claude/Cursor)" icon="plug" href="#mcp-server">
		Give your agent a way to reference and search through smaran docs.
  </Card>
...
```bash
npx -y install-mcp@latest https://smaran.ai/docs/mcp --client claude-code --oauth=no -y
```
...
Note: You can always reference the documentation by using the **SearchSmaranDocs MCP** or running a web search tool for content on **smaran.ai/docs**.

CANONICAL API SURFACE (use these, nothing else):

- Auth header: `Authorization: Bearer $SMARAN_API_KEY` — the only supported auth header
- Write content:     POST https://api.smaran.ai/v3/documents
- Search:            POST https://api.smaran.ai/v4/search
- Profile + search:  POST https://api.smaran.ai/v4/profile
- Settings:          PATCH https://api.smaran.ai/v3/settings
...
- Headers:    x-smaran-api-key, x-api-key, x-sm-user-id, x-sm-project,
...
is the single most common bug in AI-generated Smaran integrations.
...
await fetch('https://api.smaran.ai/v3/documents/file', {
  method: 'POST',
  headers: { 'Authorization': `Bearer ${process.env.SMARAN_API_KEY}` },
  body: formData
})
...
```bash
# 1. Clone repo
git clone https://github.com/smaranai/smaran.git

# 2. Copy skill
mkdir -p ~/.claude/skills
cp smaran/.claude/skills/smaran-integrate.md ~/.claude/skills/
...
<CardGroup cols={2}>
  ...
  <Card title="API Reference" icon="code" href="https://api.smaran.ai/v3/openapi">
    Complete API docs
  </Card>
</CardGroup>
