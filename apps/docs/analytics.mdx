...
description: "Observe usage, errors, and logs to monitor your Engram integration"
icon: "chart-line"
---

Monitor your Engram usage with detailed analytics on API calls, errors, and performance metrics.

## Overview

The Analytics API provides comprehensive insights into your Engram usage:
...
const usage = await fetch('https://api.engram.ai/v3/analytics/usage?period=24h', {
  headers: {
...
response = requests.get(
    'https://api.engram.ai/v3/analytics/usage',
    params={'period': '7d'},
...
curl -X GET "https://api.engram.ai/v3/analytics/usage?from=2024-01-01T00:00:00Z&to=2024-01-31T23:59:59Z" \
...
const errors = await fetch('https://api.engram.ai/v3/analytics/errors?period=24h', {
  headers: {
    'Authorization': `Bearer ${ENGRAM_API_KEY}`
  }
});
...
response = requests.get(
    'https://api.engram.ai/v3/analytics/errors?period=1h',
    headers={'Authorization': f'Bearer {ENGRAM_API_KEY}'}
)
...
curl -X GET "https://api.engram.ai/v3/analytics/errors?period=7d" \
  -H "Authorization: Bearer $ENGRAM_API_KEY"
```
...
const logs = await fetch('https://api.engram.ai/v3/analytics/logs?period=1h&statusCode=500', {
  headers: {
    'Authorization': `Bearer ${ENGRAM_API_KEY}`
  }
});
...
response = requests.get(
    'https://api.engram.ai/v3/analytics/logs',
    params={
        'keyId': 'key_abc123',
        'period': '24h'
    },
...
curl -X GET "https://api.engram.ai/v3/analytics/logs?period=1h&limit=50" \
  -H "Authorization: Bearer $ENGRAM_API_KEY"
```
