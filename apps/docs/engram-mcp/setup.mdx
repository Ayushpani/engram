...
description: 'How to set up and use Smaran MCP Server 4.0'
icon: 'settings'
---
...
If you prefer to use an API key instead of OAuth, get one from [app.smaran.ai](https://app.smaran.ai) and pass it in the `Authorization` header:
...
For a screenshot-backed walkthrough (Developer → Edit Config, `claude_desktop_config.json`, Connectors), see **[Claude Desktop](/smaran-mcp/claude-desktop)**. The recommended one-line install at the top of this page also supports Claude.
...
Or use the one-click install button at [app.smaran.ai](https://app.smaran.ai).

### Windsurf / VS Code

Configuration varies by extension. Generally, add the server URL (`https://mcp.smaran.ai/mcp`) to your MCP settings.