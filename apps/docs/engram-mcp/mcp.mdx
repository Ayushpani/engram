...
Smaran MCP Server 4.0 gives AI assistants (Claude, Cursor, Windsurf, etc.) persistent memory across conversations. Built on Cloudflare Workers with Durable Objects for scalable, persistent connections.
...
If you prefer API keys over OAuth, get one from [app.smaran.ai](https://app.smaran.ai) and pass it in the `Authorization` header:
...
You can access this in Cursor and Claude Code by just doing /context, which will give the LLMs just enough context to use and query smaran more.
...
<Card title="MCP Server Source Code" icon="github" href="https://github.com/smaranai/smaran/tree/main/apps/mcp">
