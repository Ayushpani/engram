...
Engram MCP Server 4.0 gives AI assistants (Claude, Cursor, Windsurf, etc.) persistent memory across conversations. Built on Cloudflare Workers with Durable Objects for scalable, persistent connections.
...
If you prefer API keys over OAuth, get one from [app.engram.ai](https://app.engram.ai) and pass it in the `Authorization` header:
...
You can access this in Cursor and Claude Code by just doing /context, which will give the LLMs just enough context to use and query engram more.
...
<Card title="MCP Server Source Code" icon="github" href="https://github.com/engramai/engram/tree/main/apps/mcp">
