...
<Note>
**Live Demo**: Try the Memory Router at [smaran.chat](https://smaran.chat) to see it in action.
</Note>

<Tip>
**Using Vercel AI SDK?** Check out our [AI SDK integration](/integrations/ai-sdk) for the cleanest implementation with `@smaranai/tools/ai-sdk` - it's our recommended approach for new projects.
</Tip>
...
<Steps>
  <Step title="Proxy Request">
    Your application sends requests to Smaran instead of directly to your LLM provider
  </Step>

  <Step title="Context Management">
    Smaran automatically:
...
- **Automatic Fallback**: If Smaran has issues, requests pass through directly
...
1. **Smaran API Key**: For memory management
...
| Header | Description |
|--------|-------------|
| `x-smaran-conversation-id` | Unique conversation identifier |
| `x-smaran-context-modified` | Whether context was modified (`true`/`false`) |
| `x-smaran-tokens-processed` | Number of tokens processed |
| `x-smaran-chunks-created` | New memory chunks created |
| `x-smaran-chunks-retrieved` | Memory chunks added to context |

## Error Handling

The Memory Router is designed for reliability:

- **Automatic Fallback**: If Smaran encounters an error, your request passes through unmodified
- **Error Headers**: `x-smaran-error` header provides error details
- **Zero Downtime**: Your application continues working even if memory features are unavailable

## Rate Limits & Pricing

### Rate Limits
- No Smaran-specific rate limits
