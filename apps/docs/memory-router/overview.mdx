...
<Note>
**Live Demo**: Try the Memory Router at [engram.chat](https://engram.chat) to see it in action.
</Note>

<Tip>
**Using Vercel AI SDK?** Check out our [AI SDK integration](/integrations/ai-sdk) for the cleanest implementation with `@engram/tools/ai-sdk` - it's our recommended approach for new projects.
</Tip>
...
<Steps>
  <Step title="Proxy Request">
    Your application sends requests to Engram instead of directly to your LLM provider
  </Step>
...
- **Automatic Fallback**: If Engram has issues, requests pass through directly
...
1. **Engram API Key**: For memory management
...
| Header | Description |
|--------|-------------|
| `x-engram-conversation-id` | Unique conversation identifier |
| `x-engram-context-modified` | Whether context was modified (`true`/`false`) |
| `x-engram-tokens-processed` | Number of tokens processed |
| `x-engram-chunks-created` | New memory chunks created |
| `x-engram-chunks-retrieved` | Memory chunks added to context |

## Error Handling

The Memory Router is designed for reliability:

- **Automatic Fallback**: If Engram encounters an error, your request passes through unmodified
- **Error Headers**: `x-engram-error` header provides error details
- **Zero Downtime**: Your application continues working even if memory features are unavailable

## Rate Limits & Pricing

### Rate Limits
- No Engram-specific rate limits
