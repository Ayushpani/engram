...
description: "Memory tools for OpenAI function calling with Smaran integration"
icon: "/images/openai.svg"
---

Add memory capabilities to the official OpenAI SDKs using Smaran. Two approaches available:

1. **`withSmaran` wrapper** - Automatic memory injection into system prompts (zero-config)
...
<Tip>
**New to Smaran?** Start with `withSmaran` for the simplest integration. It automatically injects relevant memories into your prompts.
</Tip>

<CardGroup>
<Card title="Smaran tools on npm" icon="npm" href="https://www.npmjs.com/package/@smaranai/tools">
    Check out the NPM page for more details
</Card>
<Card title="Smaran AI SDK" icon="python" href="https://pypi.org/project/smaran-openai-sdk/">
...
```bash
npm install @smaranai/tools openai
```
...
const client = withSmaran(openai, {
...
```typescript
const client = withSmaran(openai, { containerTag: "user-123", customId: "conv-456", mode: "full" })
...
import { smaranTools } from "@smaranai/tools/openai"
...
import {
  ...
  createAddMemoryTool
} from "@smaranai/tools/openai"

const searchTool = createSearchMemoriesTool(process.env.SMARAN_API_KEY!)
const addTool = createAddMemoryTool(process.env.SMARAN_API_KEY!)
...
import OpenAI from "openai"
import { getToolDefinitions, createToolCallExecutor } from "@smaranai/tools/openai"
...
async def safe_chat():
    try:
        client = openai.AsyncOpenAI()
        tools = SmaranTools(api_key="your-api-key")
...
import OpenAI from "openai"
import { getToolDefinitions } from "@smaranai/tools/openai"
...
SMARAN_API_KEY=your_smaran_key
OPENAI_API_KEY=your_openai_key
SMARAN_BASE_URL=https://custom-endpoint.com  # optional
```
