...
description: "Memory tools for OpenAI function calling with Engram integration"
icon: "/images/openai.svg"
---

Add memory capabilities to the official OpenAI SDKs using Engram. Two approaches available:

1. **`withEngram` wrapper** - Automatic memory injection into system prompts (zero-config)
...
<Tip>
**New to Engram?** Start with `withEngram` for the simplest integration. It automatically injects relevant memories into your prompts.
</Tip>
...
<Card title="Engram AI SDK" icon="python" href="https://pypi.org/project/engram-openai-sdk/">
...
```bash
npm install @engram/tools openai
```
...
```typescript
const client = withEngram(openai, { containerTag: "user-123", customId: "conv-456", mode: "full" })
...
import { engramTools } from "@engram/tools/openai"
...
import {
  ...
  createAddMemoryTool
} from "@engram/tools/openai"

const searchTool = createSearchMemoriesTool(process.env.ENGRAM_API_KEY!)
const addTool = createAddMemoryTool(process.env.ENGRAM_API_KEY!)
...
import OpenAI from "openai"
import { getToolDefinitions, createToolCallExecutor } from "@engram/tools/openai"
...
async def safe_chat():
    try:
        client = openai.AsyncOpenAI()
        tools = EngramTools(api_key="your-api-key")
...
import OpenAI from "openai"
import { getToolDefinitions } from "@engram/tools/openai"
...
ENGRAM_API_KEY=your_engram_key
OPENAI_API_KEY=your_openai_key
ENGRAM_BASE_URL=https://custom-endpoint.com  # optional
```
