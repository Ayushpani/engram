...
title: "n8n"
sidebarTitle: "n8n"
description: "Automate knowledge management with Smaran in n8n workflows"
icon: "workflow"
---

Connect Smaran to your n8n workflows to build intelligent automation workflows and agents that leverage your full knowledge base.
...
- Smaran API key ([get one here](https://console.smaran.com/settings))
- Basic understanding of n8n workflows

### Setting Up the HTTP Request Node

The Smaran integration in n8n uses the HTTP Request node to interact with the Smaran API. Here's how to configure it:

1. Add an **HTTP Request** node to your workflow (Core > HTTP Request)
![](/images/core-http-req.png)
2. Set the **Method** to `POST`
3. Set the **URL** to the appropriate Smaran API endpoint:
   - Add memory: `https://api.smaran.ai/v3/documents`
   - Search memories: `https://api.smaran.ai/v4/search`
4. For authentication, select **Generic Credential Type** and then **Bearer Auth**
5. Click on **Create New Credential** and paste the Smaran API Key in the Bearer Token field.
...
In this tutorial, we'll create a workflow that automatically adds every email from Gmail to your Smaran knowledge base. We'll use the HTTP Request node to send email data to Smaran's API, creating a searchable archive of all your communications.

### Adding Gmail Emails to Smaran
...
2. **Method**: `POST`
3. **URL**: `https://api.smaran.ai/v3/documents`
4. Select your auth credentials you created with the Smaran API Key.

#### Step 3: Format Email Data for Smaran
...
4. Verify in Smaran that the email appears in search results

Refer to the API Reference tab to learn more about other smaran API endpoints.