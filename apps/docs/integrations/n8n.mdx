...
title: "n8n"
sidebarTitle: "n8n"
description: "Automate knowledge management with Engram in n8n workflows"
icon: "workflow"
---

Connect Engram to your n8n workflows to build intelligent automation workflows and agents that leverage your full knowledge base.
...
- Engram API key ([get one here](https://console.engram.com/settings))
- Basic understanding of n8n workflows

### Setting Up the HTTP Request Node

The Engram integration in n8n uses the HTTP Request node to interact with the Engram API. Here's how to configure it:

1. Add an **HTTP Request** node to your workflow (Core > HTTP Request)
![](/images/core-http-req.png)
2. Set the **Method** to `POST`
3. Set the **URL** to the appropriate Engram API endpoint:
   - Add memory: `https://api.engram.ai/v3/documents`
   - Search memories: `https://api.engram.ai/v4/search`
4. For authentication, select **Generic Credential Type** and then **Bearer Auth**
5. Click on **Create New Credential** and paste the Engram API Key in the Bearer Token field.
...
In this tutorial, we'll create a workflow that automatically adds every email from Gmail to your Engram knowledge base. We'll use the HTTP Request node to send email data to Engram's API, creating a searchable archive of all your communications.

### Adding Gmail Emails to Engram
...
2. **Method**: `POST`
3. **URL**: `https://api.engram.ai/v3/documents`
4. Select your auth credentials you created with the Engram API Key.

#### Step 3: Format Email Data for Engram
...
4. Verify in Engram that the email appears in search results

Refer to the API Reference tab to learn more about other engram API endpoints.