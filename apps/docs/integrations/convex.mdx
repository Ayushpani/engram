...
title: "Convex"
sidebarTitle: "Convex"
description: "Add persistent memory to Convex apps with Smaran"
icon: "database"
---

Convex apps don't have built-in memory for AI. Smaran fixes that. You get a memory layer that stores conversations, builds user profiles, and gives your AI context about who it's talking to.
...
```bash
npm install smaran convex
```

For the AI chat example, also install the AI SDK packages:

```bash
npm install @smaranai/tools @ai-sdk/openai ai
```

Set up your environment variable in Convex:

```bash
npx convex env set SMARAN_API_KEY your-smaran-api-key
```
...
Create simple helper functions for each Smaran operation:
...
A chat endpoint using the Smaran AI SDK middleware. It automatically injects context and saves memories.
...
export const chat = action({
  ...
  handler: async (ctx, { userId, message }) => {
    ...
    const model = withSmaran(openai("gpt-4o-mini"), {
      containerTag: userId,
...
// Add memory to both Smaran and Convex
export const addMemory = action({
  args: { userId: v.string(), content: v.string() },
  handler: async (ctx, { userId, content }) => {
    // Add to Smaran
    ...
    // fails after the Smaran write succeeds, the two stores will be out of sync.
