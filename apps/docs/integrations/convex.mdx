...
title: "Convex"
sidebarTitle: "Convex"
description: "Add persistent memory to Convex apps with Engram"
icon: "database"
---

Convex apps don't have built-in memory for AI. Engram fixes that. You get a memory layer that stores conversations, builds user profiles, and gives your AI context about who it's talking to.
...
```bash
npm install engram convex
```

For the AI chat example, also install the AI SDK packages:

```bash
npm install @engram/tools @ai-sdk/openai ai
```

Set up your environment variable in Convex:

```bash
npx convex env set ENGRAM_API_KEY your-engram-api-key
```
...
Create simple helper functions for each Engram operation:
...
const memory = new Engram({ apiKey: process.env.ENGRAM_API_KEY });
...
A chat endpoint using the Engram AI SDK middleware. It automatically injects context and saves memories.
...
export const chat = action({
  ...
  handler: async (ctx, { userId, message }) => {
    ...
    const model = withEngram(openai("gpt-4o-mini"), {
      containerTag: userId,
...
// Add memory to both Engram and Convex
export const addMemory = action({
  args: { userId: v.string(), content: v.string() },
  handler: async (ctx, { userId, content }) => {
    // Add to Engram
    ...
    // fails after the Engram write succeeds, the two stores will be out of sync.
