...
description: "OpenClaw Smaran Plugin — works across Telegram, WhatsApp, Discord, Slack, and more"
...
[OpenClaw](https://github.com/smaranai/openclaw-smaran) is a multi-platform AI messaging gateway that connects to WhatsApp, Telegram, Discord, Slack, iMessage, and other messaging channels. The Smaran plugin gives OpenClaw memory across every channel.
...
```bash
openclaw plugins install @smaran/openclaw-smaran
```
...
```bash
openclaw smaran setup
```

Enter your API key from [console.smaran.ai](https://console.smaran.ai). That's it.
...
<AccordionGroup>
  <Accordion title="Advanced Setup">
    ...
    ```bash
    openclaw smaran setup-advanced
    ```
...
- **Auto-Recall** — Before every AI turn, Smaran is queried for relevant memories and the user's profile. These are injected as context so the AI sees preferences, facts, and semantically similar past conversations.
- **Auto-Capture** — After every AI turn, the conversation exchange is sent to Smaran for extraction and long-term storage. Smaran handles deduplication and profile building.
...
| Tool | Description |
|------|-------------|
| `smaran_store` | Save information to long-term memory. |
| `smaran_search` | Search memories by query with similarity scores. |
| `smaran_forget` | Delete a memory by query or ID. |
| `smaran_profile` | View the user profile — persistent facts and recent context. |
...
```bash
openclaw smaran setup              # Configure API key
openclaw smaran setup-advanced     # Configure all options
openclaw smaran status             # View current configuration
openclaw smaran search <query>     # Search memories
openclaw smaran profile            # View user profile
openclaw smaran wipe               # Delete all memories (requires confirmation)
```
      ...
      "plugins": {
        "entries": {
          "openclaw-smaran": {
            "enabled": true,
            "config": {
              "apiKey": "${SMARAN_OPENCLAW_API_KEY}",
...
<AccordionGroup>
  <Accordion title="How do I install OpenClaw with Smaran?">
    ...
    3. When prompted, paste your API key from [console.smaran.ai](https://console.smaran.ai). The key starts with `sm_`.
    ...
    ```bash
    openclaw smaran status
    ```

    That's it — Smaran is now connected and will automatically recall and capture memories across all your channels.
  </Accordion>

  <Accordion title="I have Twitter bookmarks in a separate container. How do I use them with Smaran?">
  ...
  <Accordion title="I'm not technical. How can I set up advanced mode?">
    ...
    ```
    What are all the available configurations for the Smaran plugin advanced mode?
    ```

    The agent will list every option. Then tell it what you want:

    ```
    Set up my Smaran plugin with these settings:
    - Auto recall: true
    - Auto capture: true
    - Enable custom container tags: true
    - Custom containers:
...
<CardGroup cols={2}>
  <Card title="GitHub Repository" icon="github" href="https://github.com/smaranai/openclaw-smaran">
    Source code, issues, and detailed README.
  </Card>

  <Card title="Claude Memory Tool" icon="brain" href="/integrations/claude-memory">
    Use Claude's native memory tool with Smaran as backend.
  </Card>
</CardGroup>
