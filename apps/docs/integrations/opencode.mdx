...
description: "OpenCode Engram Plugin — persistent memory across coding sessions"
...
[OpenCode-Engram](https://github.com/engramai/opencode-engram) is an OpenCode plugin that gives your AI persistent memory across sessions. Your agent remembers what you worked on — across sessions, across projects.
...
<Tabs>
  <Tab title="macOS / Linux (zsh)">
    ```bash
    echo 'export ENGRAM_API_KEY="sm_..."' >> ~/.zshrc
    source ~/.zshrc
    ```
  </Tab>
  <Tab title="macOS / Linux (bash)">
    ```bash
    echo 'export ENGRAM_API_KEY="sm_..."' >> ~/.bashrc
    source ~/.bashrc
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
    [System.Environment]::SetEnvironmentVariable("ENGRAM_API_KEY", "sm_...", "User")
    ```
...
```bash
bunx opencode-engram@latest install
```

For LLM agents (non-interactive):

```bash
bunx opencode-engram@latest install --no-tui
```
...
The agent has access to a `engram` tool with these modes:
...
Create `~/.config/opencode/engram.jsonc`:

```jsonc
{
  "apiKey": "sm_...",              // Or use ENGRAM_API_KEY env var
...
```bash
tail -f ~/.opencode-engram.log
```

## Next Steps

<CardGroup cols={2}>
  <Card title="GitHub Repository" icon="github" href="https://github.com/engramai/opencode-engram">
