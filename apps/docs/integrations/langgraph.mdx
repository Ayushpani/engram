...
description: "Add persistent memory to LangGraph agents with Engram"
icon: "/images/langgraph.svg"
---

Build stateful agents with LangGraph that remember context across sessions. Engram handles memory storage and retrieval while LangGraph manages your graph-based conversation flow.

## Overview

This guide shows how to integrate Engram with LangGraph to create agents that:
...
```bash
pip install langgraph langchain-openai engram python-dotenv
```
...
Engram automatically builds user profiles from stored memories:
...
LangGraph's checkpointer handles within-session state. Engram handles cross-session memory. Use both:
...
# Handled by Engram in your nodes
```
