...
description: "Add persistent memory to LangGraph agents with Smaran"
icon: "/images/langgraph.svg"
---

Build stateful agents with LangGraph that remember context across sessions. Smaran handles memory storage and retrieval while LangGraph manages your graph-based conversation flow.

## Overview

This guide shows how to integrate Smaran with LangGraph to create agents that:
...
```bash
pip install langgraph langchain-openai smaran python-dotenv
```
...
Smaran automatically builds user profiles from stored memories:
...
LangGraph's checkpointer handles within-session state. Smaran handles cross-session memory. Use both:
...
# Handled by Smaran in your nodes
```
