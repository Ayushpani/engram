...
description: "Integrate Engram with VoltAgent for long-term memory in AI agents"
icon: "bolt"
---

Engram integrates with [VoltAgent](https://github.com/VoltAgent/voltagent), providing long-term memory capabilities for AI agents. Your VoltAgent applications will remember past conversations and provide personalized responses based on user history.
...
```bash
npm install @engram/tools @voltagent/core
```
...
Engram provides a `withEngram` wrapper that enhances any VoltAgent agent config with automatic memory retrieval and storage:
...
// Create an agent with Engram memory capabilities
...
When integrated with VoltAgent, Engram hooks into two lifecycle events:

### 1. Memory Retrieval (onPrepareMessages)

Before each LLM call, Engram automatically:
...
After each agent response, the conversation is saved to Engram for future retrieval. This requires a `customId` to be set.
...
const configWithMemory = withEngram({
  ...
  // API
  apiKey: "sk-...",                // Falls back to ENGRAM_API_KEY env var
