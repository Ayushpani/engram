...
description: "Integrate Smaran with VoltAgent for long-term memory in AI agents"
icon: "bolt"
---

Smaran integrates with [VoltAgent](https://github.com/VoltAgent/voltagent), providing long-term memory capabilities for AI agents. Your VoltAgent applications will remember past conversations and provide personalized responses based on user history.
...
```bash
npm install @smaranai/tools @voltagent/core
```
...
Smaran provides a `withSmaran` wrapper that enhances any VoltAgent agent config with automatic memory retrieval and storage:
...
// Create an agent with Smaran memory capabilities
...
When integrated with VoltAgent, Smaran hooks into two lifecycle events:

### 1. Memory Retrieval (onPrepareMessages)

Before each LLM call, Smaran automatically:
...
After each agent response, the conversation is saved to Smaran for future retrieval. This requires a `customId` to be set.
...
const configWithMemory = withSmaran({
  ...
  // API
  apiKey: "sk-...",                // Falls back to SMARAN_API_KEY env var
