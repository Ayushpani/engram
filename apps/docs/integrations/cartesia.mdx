...
description: "Integrate Engram with Cartesia for conversational memory in voice AI agents"
icon: "/images/cartesia.svg"
---

Engram integrates with [Cartesia](https://cartesia.ai/agents), providing long-term memory capabilities for voice AI agents. Your Cartesia applications will remember past conversations and provide personalized responses based on user history.

## Installation

To use Engram with Cartesia, install the required dependencies:
...
You can obtain an API key from [console.engram.ai](https://console.engram.ai).

## Configuration

Engram integration is provided through the `EngramCartesiaAgent` wrapper class:
...
The `EngramCartesiaAgent` wraps your existing `LlmAgent` to add memory capabilities:
...
When integrated with Cartesia Line, Engram provides two key functionalities:

### 1. Memory Retrieval

When a `UserTurnEnded` event is detected, Engram retrieves relevant memories:
...
Conversations are automatically stored in Engram (non-blocking) for future retrieval.
...
```python
EngramCartesiaAgent.MemoryConfig(
...
EngramCartesiaAgent(
    ...
    api_key=os.getenv("ENGRAM_API_KEY"),  # Optional: defaults to env var
...
The SDK **automatically groups all messages from the same conversation** into a single Engram document using `custom_id`:
...
**How it works:**
- The `custom_id` parameter groups all messages into the same Engram document
...
Here's a complete example of a Cartesia Line voice agent with Engram integration:
