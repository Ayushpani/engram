...
description: "Integrate Smaran with Cartesia for conversational memory in voice AI agents"
icon: "/images/cartesia.svg"
---

Smaran integrates with [Cartesia](https://cartesia.ai/agents), providing long-term memory capabilities for voice AI agents. Your Cartesia applications will remember past conversations and provide personalized responses based on user history.

## Installation

To use Smaran with Cartesia, install the required dependencies:
...
You can obtain an API key from [console.smaran.ai](https://console.smaran.ai).

## Configuration

Smaran integration is provided through the `SmaranCartesiaAgent` wrapper class:
...
The `SmaranCartesiaAgent` wraps your existing `LlmAgent` to add memory capabilities:
...
When integrated with Cartesia Line, Smaran provides two key functionalities:

### 1. Memory Retrieval

When a `UserTurnEnded` event is detected, Smaran retrieves relevant memories:
...
Conversations are automatically stored in Smaran (non-blocking) for future retrieval.
...
```python
SmaranCartesiaAgent.MemoryConfig(
...
SmaranCartesiaAgent(
    ...
    api_key=os.getenv("SMARAN_API_KEY"),  # Optional: defaults to env var
...
The SDK **automatically groups all messages from the same conversation** into a single Smaran document using `custom_id`:
...
**How it works:**
- The `custom_id` parameter groups all messages into the same Smaran document
...
Here's a complete example of a Cartesia Line voice agent with Smaran integration:
