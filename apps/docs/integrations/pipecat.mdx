...
description: "Integrate Engram with Pipecat for conversational memory in voice AI agents"
icon: "/images/pipecat.svg"
---

Engram integrates with [Pipecat](https://github.com/pipecat-ai/pipecat), providing long-term memory capabilities for voice AI agents. Your Pipecat applications will remember past conversations and provide personalized responses based on user history.

## Installation

To use Engram with Pipecat, install the required dependencies:
...
Engram integration is provided through the `EngramPipecatService` class in Pipecat:

```python
from engram_pipecat import EngramPipecatService
from engram_pipecat.service import InputParams
...
The `EngramPipecatService` should be positioned between your context aggregator and LLM service in the Pipecat pipeline:
...
When integrated with Pipecat, Engram provides two key functionalities:

### 1. Memory Retrieval

When a user message is detected, Engram retrieves relevant memories:
...
Here's a complete example of a Pipecat voice agent with Engram integration:
...
For a complete example using Gemini Live speech-to-speech with Engram, check out the reference implementation:

<Card
  title="Pipecat Memory Example"
  icon="github"
  href="https://github.com/engramai/pipecat-memory"
>
