...
description: "Integrate Smaran with Pipecat for conversational memory in voice AI agents"
icon: "/images/pipecat.svg"
---

Smaran integrates with [Pipecat](https://github.com/pipecat-ai/pipecat), providing long-term memory capabilities for voice AI agents. Your Pipecat applications will remember past conversations and provide personalized responses based on user history.

## Installation

To use Smaran with Pipecat, install the required dependencies:
...
Smaran integration is provided through the `SmaranPipecatService` class in Pipecat:

```python
from smaran_pipecat import SmaranPipecatService
from smaran_pipecat.service import InputParams
...
The `SmaranPipecatService` should be positioned between your context aggregator and LLM service in the Pipecat pipeline:
...
When integrated with Pipecat, Smaran provides two key functionalities:

### 1. Memory Retrieval

When a user message is detected, Smaran retrieves relevant memories:
...
Here's a complete example of a Pipecat voice agent with Smaran integration:
...
For a complete example using Gemini Live speech-to-speech with Smaran, check out the reference implementation:

<Card
  title="Pipecat Memory Example"
  icon="github"
  href="https://github.com/smaranai/pipecat-memory"
>
