...
Memory Graph is a React component that visualizes your Smaran documents and memories as an interactive network. Documents appear as rectangular nodes, memories as hexagonal nodes, and connections between them show relationships and similarity.

<Card title="@smaranai/memory-graph on npm" icon="npm" href="https://www.npmjs.com/package/@smaranai/memory-graph">
...
Create an API route to fetch documents from Smaran:
...
export async function GET() {
  const response = await fetch('https://api.smaran.ai/v3/documents/documents', {
    method: 'POST',
    headers: {
...
<Warning>
  Never expose your Smaran API key to the client. Always fetch data through your backend.
</Warning>
...
```typescript
import { useGraphData, useGraphInteractions } from '@smaranai/memory-graph';
```

### Constants

```typescript
import { colors, GRAPH_SETTINGS, LAYOUT_CONSTANTS } from '@smaranai/memory-graph';
```
