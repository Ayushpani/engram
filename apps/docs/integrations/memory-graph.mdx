...
Memory Graph is a React component that visualizes your Engram documents and memories as an interactive network. Documents appear as rectangular nodes, memories as hexagonal nodes, and connections between them show relationships and similarity.

<Card title="@smaran/memory-graph on npm" icon="npm" href="https://www.npmjs.com/package/@smaran/memory-graph">
...
Create an API route to fetch documents from Engram:
...
export async function GET() {
  const response = await fetch('https://api.engram.ai/v3/documents/documents', {
    method: 'POST',
    headers: {
...
<Warning>
  Never expose your Engram API key to the client. Always fetch data through your backend.
</Warning>
...
```typescript
import { useGraphData, useGraphInteractions } from '@smaran/memory-graph';
```

### Constants

```typescript
import { colors, GRAPH_SETTINGS, LAYOUT_CONSTANTS } from '@smaran/memory-graph';
```
