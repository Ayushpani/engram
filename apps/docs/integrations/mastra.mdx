...
title: "Mastra"
sidebarTitle: "Mastra"
description: "Add persistent memory to Mastra AI agents with Smaran processors"
icon: "/images/mastra-icon.svg"
---

Integrate Smaran with [Mastra](https://mastra.ai) to give your AI agents persistent memory. Use the `withSmaran` wrapper for zero-config setup or processors for fine-grained control.

<Note>
  Migrating to v2 from 1.4.x? Check the [migration guide](/migration/tools-v2-upgrade).
</Note>

<Card title="@smaranai/tools on npm" icon="npm" href="https://www.npmjs.com/package/@smaranai/tools">
...
```bash
npm install @smaranai/tools @mastra/core
```

## Quick Start

Wrap your agent config with `withSmaran` to add memory capabilities:
...
<Note>
  **Memory saving is enabled by default.** Conversations are automatically saved to Smaran. To disable saving:
...
1. **Input Processor** - Fetches relevant memories from Smaran and injects them into the system prompt before the LLM call
2. **Output Processor** - Optionally saves the conversation to Smaran after generation completes
...
sequenceDiagram
    ...
    InputProcessor->>Smaran: Fetch memories
    Smaran-->>InputProcessor: Return memories
    InputProcessor->>Agent: Inject into system prompt
    Agent->>LLM: Generate response
    LLM-->>Agent: Return response
    Agent->>OutputProcessor: Process output
    OutputProcessor->>Smaran: Save conversation (if enabled)
...
| `apiKey` | `string` | `SMARAN_API_KEY` env | Your Smaran API key |
| `baseUrl` | `string` | `https://api.smaran.ai` | Custom API endpoint |
...
```typescript
const agent = new Agent(withSmaran(config, {
  containerTag: "user-123",
  customId: "conv-456",
  mode: "profile",
}))
...
import { Agent } from "@mastra/core/agent"
import { withSmaran } from "@smaranai/tools/mastra"
import type { MemoryPromptData } from "@smaranai/tools/mastra"
...
// Console output:
// [smaran] Starting memory search { containerTag: "user-123", mode: "profile" }
// [smaran] Found 5 memories
// [smaran] Injected memories into system prompt { length: 1523 }
```
...
```typescript
// Smaran processors are merged correctly:
// - Input: [smaran, myLogging] (smaran runs first)
// - Output: [myAnalytics, smaran] (smaran runs last)
const agent = new Agent(withSmaran(
  {
    id: "my-assistant",
    model: openai("gpt-4o"),
...
```typescript
function withSmaran<T extends AgentConfig>(
  config: T,
...
**Returns:** Enhanced config with Smaran processors injected
...
```typescript
function createSmaranProcessor(
  options: SmaranMastraOptions
...
```typescript
function createSmaranOutputProcessor(
  options: SmaranMastraOptions
...
```typescript
function createSmaranProcessors(
  options: SmaranMastraOptions
): {
...
```bash
SMARAN_API_KEY=your_smaran_key
```
...
const agent = new Agent(withSmaran(
  { id: "my-assistant", model: openai("gpt-4o"), instructions: "..." },
  {
    containerTag: "user-123",
    customId: "conv-456",
    apiKey: undefined,  // Will check SMARAN_API_KEY env
  }
))
// Error: SMARAN_API_KEY is not set
```
