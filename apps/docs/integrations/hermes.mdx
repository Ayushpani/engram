...
title: "Hermes"
sidebarTitle: "Hermes"
description: "Hermes Engram Plugin — semantic memory, profiles, and search across Telegram, Discord, Slack, CLI, and more"
icon: "/images/hermes.svg"
---


[Hermes agent](https://github.com/NousResearch/hermes-agent) ships a native **engram** memory provider: semantic long-term memory, profile recall, search, explicit memory tools, and session-aware ingest — not a bolt-on script the model might skip. Hermes runs across Telegram, Discord, Slack, WhatsApp, Signal, and the CLI from one gateway.

Hermes also includes built-in **`MEMORY.md`** and **`USER.md`**. engram adds **structure and isolation** (profile-scoped and optional multi-container tags) plus retrieval that goes beyond a single flat file.

## Get Your API Key

Create a engram API key from the [app with plugins deep link](https://app.engram.ai/?plugins=true) (or the [API Keys](https://console.engram.ai/keys) page in the console). During `hermes memory setup` you can paste it when prompted, or persist it in your environment:
...
Select **engram** when prompted and paste your API key.

Or set the provider and key manually:

```bash
hermes config set memory.provider engram
echo 'ENGRAM_API_KEY=sm_...' >> ~/.hermes/.env
```
...
| Tool | Description |
|------|-------------|
| `engram_store` | Store an explicit memory. |
| `engram_search` | Search by semantic similarity. |
| `engram_forget` | Forget a memory by ID or best-match query. |
| `engram_profile` | Retrieve persistent profile and recent context. |

## Commands

Interactive setup: pick **engram** and enter your API key.
...
These variables configure the engram provider (for example in your shell or Hermes env file):

```bash
ENGRAM_API_KEY=sm_...              # Required
ENGRAM_CONTAINER_TAG=hermes-work # Optional: overrides container tag from config
```
...
**How to enable** — In `$HERMES_HOME/engram.json`, set:
...
- **`engram_search`**, **`engram_store`**, **`engram_forget`**, and **`engram_profile`** accept an optional **`container_tag`** argument. The tag must be either the **primary** `container_tag` (after template resolution) or one of **`custom_containers`**.
...
See the [upstream plugin README](https://github.com/NousResearch/hermes-agent/tree/main/plugins/memory/engram) for the exact schema and any newer options.

## Config file

Create or edit `$HERMES_HOME/engram.json`. Common keys:
...
If you run your own engram API, set **`base_url`** (and any other host-specific options) in `engram.json` or via env as documented in the [upstream plugin README](https://github.com/NousResearch/hermes-agent/tree/main/plugins/memory/engram) — alongside your key and container settings.

## Next Steps

<CardGroup cols={2}>
  <Card title="Hermes + plugin README" icon="github" href="https://github.com/NousResearch/hermes-agent/tree/main/plugins/memory/engram">
...
Questions about the API or product? [Discord](https://engram.link/discord) · [support@engram.com](mailto:support@engram.com) · [Developer docs](/intro)
