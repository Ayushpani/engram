...
title: "Hermes"
sidebarTitle: "Hermes"
description: "Hermes Smaran Plugin — semantic memory, profiles, and search across Telegram, Discord, Slack, CLI, and more"
icon: "/images/hermes.svg"
---


[Hermes agent](https://github.com/NousResearch/hermes-agent) ships a native **smaran** memory provider: semantic long-term memory, profile recall, search, explicit memory tools, and session-aware ingest — not a bolt-on script the model might skip. Hermes runs across Telegram, Discord, Slack, WhatsApp, Signal, and the CLI from one gateway.

Hermes also includes built-in **`MEMORY.md`** and **`USER.md`**. smaran adds **structure and isolation** (profile-scoped and optional multi-container tags) plus retrieval that goes beyond a single flat file.

## Get Your API Key

Create a smaran API key from the [app with plugins deep link](https://app.smaran.ai/?plugins=true) (or the [API Keys](https://console.smaran.ai/keys) page in the console). During `hermes memory setup` you can paste it when prompted, or persist it in your environment:
...
Select **smaran** when prompted and paste your API key.

Or set the provider and key manually:

```bash
hermes config set memory.provider smaran
echo 'SMARAN_API_KEY=sm_...' >> ~/.hermes/.env
```
...
| Tool | Description |
|------|-------------|
| `smaran_store` | Store an explicit memory. |
| `smaran_search` | Search by semantic similarity. |
| `smaran_forget` | Forget a memory by ID or best-match query. |
| `smaran_profile` | Retrieve persistent profile and recent context. |

## Commands

Interactive setup: pick **smaran** and enter your API key.
...
These variables configure the smaran provider (for example in your shell or Hermes env file):

```bash
SMARAN_API_KEY=sm_...              # Required
SMARAN_CONTAINER_TAG=hermes-work # Optional: overrides container tag from config
```
...
**How to enable** — In `$HERMES_HOME/smaran.json`, set:
...
- **`smaran_search`**, **`smaran_store`**, **`smaran_forget`**, and **`smaran_profile`** accept an optional **`container_tag`** argument. The tag must be either the **primary** `container_tag` (after template resolution) or one of **`custom_containers`**.
...
See the [upstream plugin README](https://github.com/NousResearch/hermes-agent/tree/main/plugins/memory/smaran) for the exact schema and any newer options.

## Config file

Create or edit `$HERMES_HOME/smaran.json`. Common keys:
...
If you run your own smaran API, set **`base_url`** (and any other host-specific options) in `smaran.json` or via env as documented in the [upstream plugin README](https://github.com/NousResearch/hermes-agent/tree/main/plugins/memory/smaran) — alongside your key and container settings.

## Next Steps

<CardGroup cols={2}>
  <Card title="Hermes + plugin README" icon="github" href="https://github.com/NousResearch/hermes-agent/tree/main/plugins/memory/smaran">
...
Questions about the API or product? [Discord](https://smaran.link/discord) · [support@smaran.com](mailto:support@smaran.com) · [Developer docs](/intro)
