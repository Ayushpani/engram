...
description: "Use Claude's native memory tool with Smaran as the backend"
icon: "/images/anthropic-1.svg"
---

Claude has a native memory tool that allows it to store and retrieve information across conversations. Smaran provides a backend implementation that maps Claude's memory commands to persistent storage.
...
Claude's memory tool uses a file-system metaphor. Smaran maps these operations to document storage:

| Claude Command | Smaran Action |
|----------------|-------------------|
