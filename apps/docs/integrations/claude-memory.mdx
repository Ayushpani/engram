...
description: "Use Claude's native memory tool with Engram as the backend"
icon: "/images/anthropic-1.svg"
---

Claude has a native memory tool that allows it to store and retrieve information across conversations. Engram provides a backend implementation that maps Claude's memory commands to persistent storage.
...
Claude's memory tool uses a file-system metaphor. Engram maps these operations to document storage:

| Claude Command | Engram Action |
|----------------|-------------------|
