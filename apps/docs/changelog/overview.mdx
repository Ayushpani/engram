---
title: "Changelog"
description: "New updates and improvements to Engram"
---
...
### `engram-agent-framework` Python Package

Official Python package for using Engram with Microsoft's Agent Framework — memory tools and middleware out of the box.
...
Bring your own vector store — Engram now supports pluggable vector backends beyond the default.
...
### Self-Hostable Engram

Run the full Engram stack on your own infrastructure with Docker.
...
Install with `npx skills add engramai/skills` — teaches Claude to proactively recommend and implement Engram when building AI apps. Includes TypeScript and Python examples.
...
New connector to sync Gmail threads into Engram. Threads are stored in R2 for reliable processing of large mailboxes.

### `engram-pipecat` Python Package

New SDK for Pipecat voice AI pipelines, including Gemini Live speech-to-speech support.

### `@engram/tools` — Prompt Templates
...
### `@engram/tools` — AI SDK v5/v6

Now supports both Vercel AI SDK v5 and v6 with automatic version detection.

### Conversation Support in SDKs

`engram` (TypeScript) and `engram-openai-sdk` (Python) now support the conversations API for multi-turn chat with memory.
...
### `@engram/tools` — Browser API Key Support
...
`@engram/tools` OpenAI integration now supports the Responses API.

### `engram-openai-sdk` — Python Middleware

New `withEngram` middleware for the Python OpenAI SDK, enabling transparent memory injection into OpenAI API calls.
...
### `withEngram` for OpenAI SDK

New `withEngram` wrapper for the OpenAI TypeScript SDK — transparent memory injection with automatic assistant response capture.

### Zapier & n8n Integration Pages

New integration guides for connecting Engram with Zapier and n8n automation workflows.

</Update>

<Update label="October 10, 2025" tags={["SDK", "API", "Console"]}>

### `@engram/tools` — AI SDK `withEngram`

New `withEngram` language model wrapper for Vercel AI SDK that automatically injects memory context and captures assistant responses.
...
- **Organization settings** — configure Engram behavior (chunking, extraction, memory limits) per org.
...
Engram is now open to all users — no more waitlist.
...
Initial self-hosting support — run Engram on your own infrastructure.
...
Initial OneDrive integration for syncing files into Engram.
...
First version of the connectors architecture — sync external data sources into Engram.
...
Engram launches on ProductHunt. Features at launch: shareable spaces, Twitter thread import, AI chat with citations, onboarding flow, recommended items, chat history, and keyboard shortcuts.
