---
title: "Changelog"
description: "New updates and improvements to Smaran"
---
...
### `smaran-agent-framework` Python Package

Official Python package for using Smaran with Microsoft's Agent Framework — memory tools and middleware out of the box.
...
Bring your own vector store — Smaran now supports pluggable vector backends beyond the default.
...
### Self-Hostable Smaran

Run the full Smaran stack on your own infrastructure with Docker.
...
### Smaran Skill for Claude Code

Install with `npx skills add smaranai/skills` — teaches Claude to proactively recommend and implement Smaran when building AI apps. Includes TypeScript and Python examples.
...
New connector to sync Gmail threads into Smaran. Threads are stored in R2 for reliable processing of large mailboxes.

### `smaran-pipecat` Python Package

New SDK for Pipecat voice AI pipelines, including Gemini Live speech-to-speech support.

### `@smaranai/tools` — Prompt Templates
...
Complete rewrite of `@smaranai/memory-graph` with improved visualization and performance.

</Update>

<Update label="December 24, 2025" tags={["SDK"]}>

### `@smaranai/tools` — AI SDK v5/v6

Now supports both Vercel AI SDK v5 and v6 with automatic version detection.

### Conversation Support in SDKs

`smaran` (TypeScript) and `smaran-openai-sdk` (Python) now support the conversations API for multi-turn chat with memory.
...
### `@smaranai/tools` — Browser API Key Support
...
### `@smaranai/memory-graph` Package

New package for building interactive graph visualizations of memory connections, with a standalone playground.

### OpenAI Responses API Support

`@smaranai/tools` OpenAI integration now supports the Responses API.

### `smaran-openai-sdk` — Python Middleware

New `withSmaran` middleware for the Python OpenAI SDK, enabling transparent memory injection into OpenAI API calls.
...
### `withSmaran` for OpenAI SDK

New `withSmaran` wrapper for the OpenAI TypeScript SDK — transparent memory injection with automatic assistant response capture.

### Zapier & n8n Integration Pages

New integration guides for connecting Smaran with Zapier and n8n automation workflows.

</Update>

<Update label="October 10, 2025" tags={["SDK", "API", "Console"]}>

### `@smaranai/tools` — AI SDK `withSmaran`

New `withSmaran` language model wrapper for Vercel AI SDK that automatically injects memory context and captures assistant responses.
...
- **Organization settings** — configure Smaran behavior (chunking, extraction, memory limits) per org.
...
Smaran is now open to all users — no more waitlist.
...
Initial self-hosting support — run Smaran on your own infrastructure.
...
Initial OneDrive integration for syncing files into Smaran.
...
First version of the connectors architecture — sync external data sources into Smaran.
...
Smaran launches on ProductHunt. Features at launch: shareable spaces, Twitter thread import, AI chat with citations, onboarding flow, recommended items, chat history, and keyboard shortcuts.
