...
- **`@smaran/tools` v1.4.1:** Now exposes raw `searchResults` in `MemoryPromptData`, giving full control over how retrieved memories are formatted in prompts.
...
- **OpenAI SDK Backfill:** Improved compatibility across `smaran-openai-sdk` (Python) and `@smaran/tools` (TypeScript) OpenAI integrations.
...
- **`@smaran/tools` — Strict Mode:** Strict mode support for OpenAI function calling, ensuring schema-validated tool calls.
...
- **`@smaran/tools` — Prompt Templates:** Customize how memory context is formatted in AI SDK integrations with the new `promptTemplate` option.
...
- **Memory Graph Revamp:** Complete rewrite of `@smaran/memory-graph` with improved visualization and performance.

## December 24, 2025

- **`@smaran/tools` — Vercel AI SDK v5/v6:** Now supports both Vercel AI SDK v5 and v6, with automatic version detection.
...
- **`@smaran/tools` — Browser API Key Support:** `apiKey` can now be passed via options instead of relying on `process.env`, enabling browser-based usage of the tools package.

## November 17, 2025

- **Web Crawler Connector:** New connector to crawl and index entire websites with configurable depth and URL patterns.
- **`@smaran/memory-graph` Package:** New package for building interactive graph visualizations of memory connections, with a standalone playground.
- **OpenAI Responses API Support:** `@smaran/tools` OpenAI integration now supports the Responses API.
...
- **AI SDK Integration:** New `@smaran/tools/ai-sdk` package for native Vercel AI SDK integration with memory tools and infinite chat capabilities
