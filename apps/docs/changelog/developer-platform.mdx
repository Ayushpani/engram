...
- **Smaran CLI:** New command-line tool for managing memories, documents, profiles, tags, connectors, and API keys directly from the terminal.
...
- **`@smaranai/tools` v1.4.1:** Now exposes raw `searchResults` in `MemoryPromptData`, giving full control over how retrieved memories are formatted in prompts.
...
- **`smaran-agent-framework` Python Package:** Official Python package for using Smaran with Microsoft's Agent Framework — memory tools and middleware out of the box.
- **OpenAI SDK Backfill:** Improved compatibility across `smaran-openai-sdk` (Python) and `@smaranai/tools` (TypeScript) OpenAI integrations.
...
- **ViaSocket Integration:** New integration guide for connecting Smaran with ViaSocket automation workflows.

## March 2, 2026

- **Configurable Vector Stores:** Bring your own vector store — Smaran now supports pluggable vector backends beyond the default.
- **List Memories Endpoint:** New `GET /v3/documents` endpoint with pagination, filtering by container tag, status, and metadata.

## February 26, 2026

- **Self-Hostable Smaran:** Run the full Smaran stack on your own infrastructure with Docker.
...
- **Smaran Skill for Claude Code:** Install with `npx skills add smaranai/skills` — teaches Claude to proactively recommend and implement Smaran when building AI apps that need persistent memory, user profiles, or semantic search. Includes ready-to-use TypeScript and Python examples.
...
- **Claude Code Integration:** Official integration page for using Smaran as persistent memory in Claude Code.
...
- **`@smaranai/tools` — Strict Mode:** Strict mode support for OpenAI function calling, ensuring schema-validated tool calls.
...
- **Gmail Connector:** New connector to sync Gmail threads into Smaran. Threads are stored in R2 for reliable processing of large mailboxes.
...
- **`smaran-pipecat` Python Package:** New SDK for integrating Smaran with Pipecat voice AI pipelines, including Gemini Live speech-to-speech support.
- **`@smaranai/tools` — Prompt Templates:** Customize how memory context is formatted in AI SDK integrations with the new `promptTemplate` option.
...
- **Memory Graph Revamp:** Complete rewrite of `@smaranai/memory-graph` with improved visualization and performance.

## December 24, 2025

- **`@smaranai/tools` — Vercel AI SDK v5/v6:** Now supports both Vercel AI SDK v5 and v6, with automatic version detection.
- **Conversation Support in SDKs:** `smaran` (TypeScript) and `smaran-openai-sdk` (Python) now support the conversations API for multi-turn chat with memory.
...
- **`@smaranai/tools` — Browser API Key Support:** `apiKey` can now be passed via options instead of relying on `process.env`, enabling browser-based usage of the tools package.

## November 17, 2025

- **Web Crawler Connector:** New connector to crawl and index entire websites with configurable depth and URL patterns.
- **`@smaranai/memory-graph` Package:** New package for building interactive graph visualizations of memory connections, with a standalone playground.
- **OpenAI Responses API Support:** `@smaranai/tools` OpenAI integration now supports the Responses API.
- **`smaran-openai-sdk` — Python Middleware:** New `withSmaran` middleware for the Python OpenAI SDK, enabling transparent memory injection into OpenAI API calls.
...
- **AI SDK Integration:** New `@smaranai/tools/ai-sdk` package for native Vercel AI SDK integration with memory tools and infinite chat capabilities
...
- **MCP (Model Context Protocol) Server:** Launch of smaran MCP server for AI model integrations with full project support and auto-detection
