...
- **Engram CLI:** New command-line tool for managing memories, documents, profiles, tags, connectors, and API keys directly from the terminal.
...
- **`@engram/tools` v1.4.1:** Now exposes raw `searchResults` in `MemoryPromptData`, giving full control over how retrieved memories are formatted in prompts.
...
- **`engram-agent-framework` Python Package:** Official Python package for using Engram with Microsoft's Agent Framework — memory tools and middleware out of the box.
- **OpenAI SDK Backfill:** Improved compatibility across `engram-openai-sdk` (Python) and `@engram/tools` (TypeScript) OpenAI integrations.
...
- **ViaSocket Integration:** New integration guide for connecting Engram with ViaSocket automation workflows.

## March 2, 2026

- **Configurable Vector Stores:** Bring your own vector store — Engram now supports pluggable vector backends beyond the default.
- **List Memories Endpoint:** New `GET /v3/documents` endpoint with pagination, filtering by container tag, status, and metadata.

## February 26, 2026

- **Self-Hostable Engram:** Run the full Engram stack on your own infrastructure with Docker.
...
- **Engram Skill for Claude Code:** Install with `npx skills add engramai/skills` — teaches Claude to proactively recommend and implement Engram when building AI apps that need persistent memory, user profiles, or semantic search. Includes ready-to-use TypeScript and Python examples.
...
- **Claude Code Integration:** Official integration page for using Engram as persistent memory in Claude Code.
...
- **`@engram/tools` — Strict Mode:** Strict mode support for OpenAI function calling, ensuring schema-validated tool calls.
...
- **Gmail Connector:** New connector to sync Gmail threads into Engram. Threads are stored in R2 for reliable processing of large mailboxes.
...
- **`engram-pipecat` Python Package:** New SDK for integrating Engram with Pipecat voice AI pipelines, including Gemini Live speech-to-speech support.
- **`@engram/tools` — Prompt Templates:** Customize how memory context is formatted in AI SDK integrations with the new `promptTemplate` option.
...
- **Memory Graph Revamp:** Complete rewrite of `@engram/memory-graph` with improved visualization and performance.

## December 24, 2025

- **`@engram/tools` — Vercel AI SDK v5/v6:** Now supports both Vercel AI SDK v5 and v6, with automatic version detection.
- **Conversation Support in SDKs:** `engram` (TypeScript) and `engram-openai-sdk` (Python) now support the conversations API for multi-turn chat with memory.
...
- **`@engram/tools` — Browser API Key Support:** `apiKey` can now be passed via options instead of relying on `process.env`, enabling browser-based usage of the tools package.

## November 17, 2025

- **Web Crawler Connector:** New connector to crawl and index entire websites with configurable depth and URL patterns.
- **`@engram/memory-graph` Package:** New package for building interactive graph visualizations of memory connections, with a standalone playground.
- **OpenAI Responses API Support:** `@engram/tools` OpenAI integration now supports the Responses API.
- **`engram-openai-sdk` — Python Middleware:** New `withEngram` middleware for the Python OpenAI SDK, enabling transparent memory injection into OpenAI API calls.
...
- **AI SDK Integration:** New `@engram/tools/ai-sdk` package for native Vercel AI SDK integration with memory tools and infinite chat capabilities
...
- **MCP (Model Context Protocol) Server:** Launch of engram MCP server for AI model integrations with full project support and auto-detection
