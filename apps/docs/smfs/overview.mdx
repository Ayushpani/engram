...
**SMFS** mounts your Smaran container as a real directory. Agents read it with `ls`, `cat`, and `grep`. No SDK to learn, no client to wire up, no embeddings to think about.
...
- **Memory paths get distilled.** Files marked as memory paths are extracted and indexed by Smaran. They don't bloat the model's context.
...
- **Bidirectional sync** runs in the background. Local reads hit cache; writes push to Smaran.
...
<CardGroup cols={2}>
  ...
  <Card title="Bash Tool (TypeScript & Python)" icon="terminal" href="/smfs/bash-tool">
    For agents running serverless or at the edge. Cloudflare Workers, AWS Lambda, Vercel, Modal. A virtual bash where the filesystem is your container. Available as [`@smaran/bash`](/smfs/bash-tool) for TypeScript and [`smaran-bash`](/smfs/bash-tool-python) for Python.
  </Card>
</CardGroup>
