...
title: "Mount"
sidebarTitle: "Mount"
description: "Mount a Engram container, generate memories, and sync."
icon: "hard-drive"
---

A mount turns a Engram container into a directory on your machine. macOS uses NFSv3, Linux uses FUSE. Both are handled for you.
...
This is the part most people miss. SMFS isn't a normal filesystem. It generates **memories** from files at specific paths. Memories are extracted, summarized, and indexed by Engram.
...
By default, files named `user.md` or `memory.md` are treated as memory paths. Drop those files anywhere in your mount and Engram generates memories from them automatically.
...
- Omit the flag and Engram keeps whatever the container tag already has, falling back to `user.md` and `memory.md`.

### profile.md

Every mount has a virtual file at the root called `profile.md`. It's auto-generated, read-only, and backed by Engram. The model can `cat profile.md` to get a live digest of everything in the container without walking every file. Useful as a first call at the start of a session.

```bash
cat agent_memory/profile.md
```

You can't write to it. As the underlying memories change, Engram regenerates it.
...
Local reads hit the cache. Local writes queue and push to Engram in the background. Remote changes are pulled on a poll. This is what you get if you pass no flags.
...
Writes still push to Engram. Polling for remote changes is off. The agent sees a view that doesn't shift under it mid-task.
...
<AccordionGroup>
  ...
  <Accordion title="smfs login" icon="log-in">
    One-time auth. Prompts for your Engram API key and stores it in your global credentials. You can also pass it directly with `--key`.
