...
description: "engram-bash. The SMFS idea wrapped as a single agent tool, for Python agents and serverless runtimes."
icon: "terminal"
---

`engram-bash` is the SMFS idea wrapped as a single agent tool: `run_bash(command)`. The "filesystem" is your Engram container. Runs anywhere Python runs. AWS Lambda, Modal, Fly Machines, Cloud Run, your laptop. No mount, no FUSE, no local disk.
...
import asyncio
import os
from engram_bash import create_bash
...
- `configure_memory_paths(paths)`: scope which paths get extracted into Engram
...
| --- | --- | --- |
| `api_key` | required | Engram API key |
