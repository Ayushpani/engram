...
description: "smaran-bash. The SMFS idea wrapped as a single agent tool, for Python agents and serverless runtimes."
icon: "terminal"
---

`smaran-bash` is the SMFS idea wrapped as a single agent tool: `run_bash(command)`. The "filesystem" is your Smaran container. Runs anywhere Python runs. AWS Lambda, Modal, Fly Machines, Cloud Run, your laptop. No mount, no FUSE, no local disk.
...
import asyncio
import os
from smaran_bash import create_bash
...
- `configure_memory_paths(paths)`: scope which paths get extracted into Smaran
...
| --- | --- | --- |
| `api_key` | required | Smaran API key |
