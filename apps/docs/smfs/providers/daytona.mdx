...
Mount a Engram container inside a [Daytona](https://daytona.io) sandbox so
your agent can read and write memory using standard filesystem commands.

<Warning>
  Daytona sandboxes currently cannot reach `api.engram.ai` from their
  ...
  claude-agent-sdk` still works — but the runtime sync to Engram fails. We're
...
<Tabs>
  <Tab title="Python">
    ...
    SMFS_INSTALL = (
        ...
        "curl -sL https://github.com/engramai/smfs/releases/download/"
...
<Tabs>
  <Tab title="Python">
    ...
    sandbox = daytona.create(
        env_vars={
            "ENGRAM_API_KEY": os.environ["ENGRAM_API_KEY"],
    ...
    # Mount memory
    sandbox.process.exec("$HOME/.local/bin/smfs login --key $ENGRAM_API_KEY")
  ...
  <Tab title="TypeScript">
    ...
    const sandbox = await daytona.create({
      envVars: {
        ENGRAM_API_KEY: process.env.ENGRAM_API_KEY!,
    ...
    await sandbox.process.exec(
      "$HOME/.local/bin/smfs login --key $ENGRAM_API_KEY"
    );
    await sandbox.process.exec(
      "bash -c '$HOME/.local/bin/smfs mount my_agent --ephemeral " +
      "--path /home/daytona/memory --foreground &' && sleep 3"
