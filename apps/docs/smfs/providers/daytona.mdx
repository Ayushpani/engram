...
Mount a Smaran container inside a [Daytona](https://daytona.io) sandbox so
your agent can read and write memory using standard filesystem commands.

<Warning>
  Daytona sandboxes currently cannot reach `api.smaran.ai` from their
  ...
  claude-agent-sdk` still works — but the runtime sync to Smaran fails. We're
...
<Tabs>
  <Tab title="Python">
    ...
    SMFS_INSTALL = (
        ...
        "curl -sL https://github.com/smaranai/smfs/releases/download/"
...
<Tabs>
  <Tab title="Python">
    ...
    sandbox = daytona.create(
        env_vars={
            "SMARAN_API_KEY": os.environ["SMARAN_API_KEY"],
    ...
    # Mount memory
    sandbox.process.exec("$HOME/.local/bin/smfs login --key $SMARAN_API_KEY")
  ...
  <Tab title="TypeScript">
    ...
    const sandbox = await daytona.create({
      envVars: {
        SMARAN_API_KEY: process.env.SMARAN_API_KEY!,
    ...
    await sandbox.process.exec(
      "$HOME/.local/bin/smfs login --key $SMARAN_API_KEY"
    );
    await sandbox.process.exec(
      "bash -c '$HOME/.local/bin/smfs mount my_agent --ephemeral " +
      "--path /home/daytona/memory --foreground &' && sleep 3"
