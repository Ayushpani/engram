...
Mount a Engram container inside an [E2B](https://e2b.dev) sandbox so your
...
<Tabs>
  <Tab title="Python">
    ...
    sbx.commands.run("smfs login --key $ENGRAM_API_KEY")
    sbx.commands.run(
  ...
  <Tab title="TypeScript">
    ...
    // Mount memory. We background the foreground daemon so this command returns,
    // then sleep briefly to let the FUSE mount come up before the agent reads it.
    await sbx.commands.run("smfs login --key $ENGRAM_API_KEY");
