...
- **Vercel Functions / serverless / edge**: there's no long-lived process to
  ...
  (`@smaran/bash`) instead. The container becomes the filesystem; no mount
  needed.
...
graph LR
    ...
    Mount -->|sync| SM["Smaran"]
```

### Vercel Functions / serverless (Bash Tool)

The agent runs inside `generateText` and accesses memory through `@smaran/bash`,
which proxies bash commands to your Smaran container over HTTP. No mount,
no FUSE, no long-lived process required.

```mermaid
graph LR
    subgraph Vercel Function
        AI["generateText()"] -->|"bash tool"| Bash["@smaran/bash"]
    end
    Bash -->|HTTPS| SM["Smaran"]
```

## Prerequisites

- A [Smaran API key](https://smaran.ai)
...
`@smaran/bash` exposes your Smaran container as a single agent tool
...
```bash
npm install @smaran/bash ai @ai-sdk/anthropic zod
```

```typescript api/agent.ts
import { generateText, tool } from "ai";
import { anthropic } from "@ai-sdk/anthropic";
import { createBash } from "@smaran/bash";
import { z } from "zod";
...
- **Pattern B**: configure memory paths once at startup with
  ...
  get distilled into Smaran memories.
