...
- **Vercel Functions / serverless / edge**: there's no long-lived process to
  ...
  (`@engram/bash`) instead. The container becomes the filesystem; no mount
  needed.
...
graph LR
    ...
    Mount -->|sync| SM["Engram"]
```

### Vercel Functions / serverless (Bash Tool)

The agent runs inside `generateText` and accesses memory through `@engram/bash`,
which proxies bash commands to your Engram container over HTTP. No mount,
no FUSE, no long-lived process required.

```mermaid
graph LR
    subgraph Vercel Function
        AI["generateText()"] -->|"bash tool"| Bash["@engram/bash"]
    end
    Bash -->|HTTPS| SM["Engram"]
```

## Prerequisites

- A [Engram API key](https://engram.ai)
...
`@engram/bash` exposes your Engram container as a single agent tool
...
```bash
npm install @engram/bash ai @ai-sdk/anthropic zod
```

```typescript api/agent.ts
import { generateText, tool } from "ai";
import { anthropic } from "@ai-sdk/anthropic";
import { createBash } from "@engram/bash";
import { z } from "zod";
...
- **Pattern B**: configure memory paths once at startup with
  ...
  get distilled into Engram memories.
