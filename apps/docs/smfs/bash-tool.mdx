...
description: "@smaran/bash. The SMFS idea wrapped as a single agent tool, for serverless and edge runtimes."
icon: "terminal"
---

`@smaran/bash` is the SMFS idea wrapped as a single agent tool: `run_bash(command)`. The "filesystem" is your Smaran container. Runs anywhere TypeScript runs. Cloudflare Workers, AWS Lambda, Vercel, Node, the browser. No mount, no FUSE, no local disk.
...
- `configureMemoryPaths(paths)`: scope which paths get extracted into Smaran
...
| --- | --- | --- |
| `apiKey` | required | Smaran API key |
