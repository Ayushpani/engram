...
description: "smaran automatically detects the content type of the document you are adding."
icon: "sparkles"
---

smaran is natively multi-modal, and can automatically detect the content type of the document you are adding.

We use the best of breed tools to extract content from URLs, and process it for optimal memory storage.

## Automatic Content Type Detection

smaran automatically detects the content type of the document you're adding. Simply pass your content to the API, and smaran will handle the rest.
...
curl https://api.smaran.ai/v3/documents \
  --request POST \
  --header 'Authorization: Bearer SMARAN_API_KEY' \
...
<Note>
  smaran uses [Markdowner](https://md.dhr.wtf) to extract content from
  URLs.
</Note>

## Supported Content Types

smaran supports a wide range of content formats to ensure versatility in memory creation:
...
<Steps>
  <Step title="Content Detection">
    smaran automatically identifies the content type based on the input provided.
  </Step>
