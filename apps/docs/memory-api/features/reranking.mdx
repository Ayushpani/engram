---
title: "Reranking"
description: "Reranked search results in engram"
...
In engram, you can enable answer rewriting by setting the `rerank` parameter to `true` in the search API.

<CodeGroup>

```bash cURL
curl https://api.engram.ai/v3/search?q=What+is+the+capital+of+France?&rerank=true \
  --request GET \
