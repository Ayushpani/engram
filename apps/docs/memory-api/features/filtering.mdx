---
title: "Filtering"
description: "Learn how to filter content while searching from smaran"
...
The graph is built on top of the Container Tags. For example, each user / tag in your smaran account will have one single graph built for them.
...
```bash cURL
curl --location 'https://api.smaran.ai/v3/documents' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer SMARAN_API_KEY' \
--data '{
...
```bash cURL
curl --location 'https://api.smaran.ai/v3/search' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer SMARAN_API_KEY' \
--data '{
    "q": "meeting",
