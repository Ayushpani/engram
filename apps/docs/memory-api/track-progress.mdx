...
const response = await fetch('https://api.engram.ai/v3/documents/processing', {
  headers: {
...
response = requests.get(
    'https://api.engram.ai/v3/documents/processing',
...
```bash cURL
curl -X GET "https://api.engram.ai/v3/documents/processing" \
...
```bash cURL
curl -X GET "https://api.engram.ai/v3/documents/doc_abc123" \
  -H "Authorization: Bearer $ENGRAM_API_KEY"
```
