...
const response = await fetch('https://api.smaran.ai/v3/documents/processing', {
  headers: {
...
response = requests.get(
    'https://api.smaran.ai/v3/documents/processing',
...
```bash cURL
curl -X GET "https://api.smaran.ai/v3/documents/processing" \
...
```bash cURL
curl -X GET "https://api.smaran.ai/v3/documents/doc_abc123" \
  -H "Authorization: Bearer $SMARAN_API_KEY"
```
