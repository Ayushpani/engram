...
By default, engram uses its own OAuth applications to connect to third-party providers. However, you can configure your own OAuth application credentials for enhanced security and control. This is particularly useful for enterprise customers who want to maintain control over their data access.
...
```
https://api.engram.ai/v3/connections/auth/callback/{provider}
```

For example, if you are using Google Drive, the redirect URL would be:

```
https://api.engram.ai/v3/connections/auth/callback/google-drive
```
...
# Example: Configure Google Drive custom OAuth credentials
curl --request PATCH \
  --url https://api.engram.ai/v3/settings \
