...
description: "Complete guide to ingesting text, URLs, files, and various content types into Engram"
---

Engram provides a powerful and flexible ingestion system that can process virtually any type of content. Whether you're adding simple text notes, web pages, PDFs, images, or complex documents from various platforms, our API handles it all seamlessly.

## Understanding the Mental Model

Before diving into the API, it's important to understand how Engram processes your content:

### Documents vs Memories

- **Documents**: Anything you put into Engram (files, URLs, text) is considered a **document**
- **Memories**: Documents are automatically chunked into smaller, searchable pieces called **memories**

When you use the "Add Memory" endpoint, you're actually adding a **document**. Engram's job is to intelligently break that document into optimal **memories** that can be searched and retrieved.
...
You can visualize this process in the [Engram Console](https://console.engram.ai) where you'll see a graph view showing how your documents are broken down into interconnected memories.

### Content Sources

Engram accepts content through three main methods:
...
<Note>
Despite the endpoint name, you're creating a **document** that Engram will automatically chunk into searchable **memories**.
</Note>
...
Got a PDF, image, or video? Upload it directly and let Engram extract the valuable content automatically.

**Endpoint:** `POST /v3/documents/file`

**What makes this powerful:** Instead of manually copying text from PDFs or transcribing videos, just upload the file. Engram handles OCR for images, transcription for videos, and intelligent text extraction for documents.

<CodeGroup>

```bash cURL
curl https://api.engram.ai/v3/documents/file \
  -H "Authorization: Bearer $ENGRAM_API_KEY" \
  -F "file=@document.pdf" \
...
const response = await fetch('https://api.engram.ai/v3/documents/file', {
  method: 'POST',
  headers: {
...
client = Engram(api_key="your_api_key")
...
response = requests.post(
    'https://api.engram.ai/v3/documents/file',
...
Engram automatically detects content types based on:
...
```bash
curl -X PATCH "https://api.engram.ai/v3/documents/abc123" \
  -H "Authorization: Bearer $ENGRAM_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
<Tabs>
  ...
  <Tab title="Python">
    ...
    from engram import Engram, BadRequestError, RateLimitError
...
<Note>
Ready to start ingesting? [Get an API key](https://console.engram.ai) now!
</Note>
