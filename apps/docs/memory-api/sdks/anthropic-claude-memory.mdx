...
description: "Enable Claude's persistent memory capabilities with Engram as the backend"
---

Enable Claude's native memory tool functionality with Engram as the persistent storage backend. Claude can automatically store and retrieve information across conversations using familiar filesystem operations.
...
- **Files**: Individual memory items stored as Engram documents
...
const memoryTool = createClaudeMemoryTool(
  process.env.ENGRAM_API_KEY!,
  {
    ...
    baseUrl: 'https://api.engram.ai', // Optional: custom API endpoint
  }
)
```
...
// This maps to searching Engram documents with path prefix
```
...
// Updates the corresponding Engram document
```
...
// Updates document customId in Engram
```
...
```typescript
import Anthropic from "@anthropic-ai/sdk"
import { createClaudeMemoryTool, type MemoryCommand } from "@engram/tools/claude-memory"
