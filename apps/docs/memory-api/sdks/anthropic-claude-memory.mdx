...
description: "Enable Claude's persistent memory capabilities with Smaran as the backend"
---

Enable Claude's native memory tool functionality with Smaran as the persistent storage backend. Claude can automatically store and retrieve information across conversations using familiar filesystem operations.
...
- **Files**: Individual memory items stored as Smaran documents
...
const memoryTool = createClaudeMemoryTool(
  process.env.SMARAN_API_KEY!,
  {
    ...
    baseUrl: 'https://api.smaran.ai', // Optional: custom API endpoint
  }
)
```
...
// This maps to searching Smaran documents with path prefix
```
...
// Updates the corresponding Smaran document
```
...
// Updates document customId in Smaran
```
...
```typescript
import Anthropic from "@anthropic-ai/sdk"
import { createClaudeMemoryTool, type MemoryCommand } from "@smaranai/tools/claude-memory"
...
**Parameters:**
- `apiKey` (string): Your Smaran API key
- `config` (optional):
