...
description: 'Learn how to use smaran with Typescript'
---
...
```ts
import fs from 'fs';
import Smaran, { toFile } from 'smaran';
...
1. Via the `SMARAN_LOG` environment variable
...
const client = new Smaran({
  logger: logger.child({ name: 'smaran' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/smaranai/sdk-ts/issues) with questions, bugs, or suggestions.
