...
description: 'Learn how to use engram with Typescript'
---
...
```ts
import fs from 'fs';
import Engram, { toFile } from 'engram';
...
1. Via the `ENGRAM_LOG` environment variable
...
const client = new Engram({
  logger: logger.child({ name: 'engram' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/engramai/sdk-ts/issues) with questions, bugs, or suggestions.
