...
Add memory capabilities to the official OpenAI SDKs using Engram's function calling tools. These plugins provide seamless integration with OpenAI's chat completions and function calling features.
...
const fetchTool = createFetchMemoryTool(process.env.ENGRAM_API_KEY!)
