...
Add memory capabilities to the official OpenAI SDKs using Smaran's function calling tools. These plugins provide seamless integration with OpenAI's chat completions and function calling features.
...
const fetchTool = createFetchMemoryTool(process.env.SMARAN_API_KEY!)
