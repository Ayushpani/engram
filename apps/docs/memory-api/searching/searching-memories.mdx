...
description: "Learn how to search for and retrieve content from smaran"
---
...
```bash cURL
curl https://api.smaran.ai/v3/search?q=machine+learning+concepts&limit=10 \
  --request GET \
...
```bash cURL
curl https://api.smaran.ai/v4/search \
  --request POST \
  --header 'Authorization: Bearer SMARAN_API_KEY' \
