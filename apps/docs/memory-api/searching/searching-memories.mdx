...
description: "Learn how to search for and retrieve content from engram"
---
...
```bash cURL
curl https://api.engram.ai/v3/search?q=machine+learning+concepts&limit=10 \
  --request GET \
...
```bash cURL
curl https://api.engram.ai/v4/search \
  --request POST \
  --header 'Authorization: Bearer ENGRAM_API_KEY' \
