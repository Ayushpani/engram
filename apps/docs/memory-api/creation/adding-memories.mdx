...
description: "Learn how to add content to smaran"
icon: "plus"
---
...
```bash cURL
curl https://api.smaran.ai/v3/documents/mem_abc123 \
  --request GET \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer SMARAN_API_KEY'
```
...
with open('/path/to/your/file.pdf', 'rb') as f:
    ...
    response = requests.post(
        ...
        headers={'Authorization': 'Bearer SMARAN_API_KEY'},
        files=files,
        data=data
    )
```
...
```bash cURL
curl https://api.smaran.ai/v3/documents/MEMORY_ID \
  --request PATCH \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer SMARAN_API_KEY' \
  --data '{
...
```typescript Typescript
await fetch(`https://api.smaran.ai/v3/documents/${memoryId}`, {
  method: "PATCH",
  headers: {
...
requests.patch(
    f'https://api.smaran.ai/v3/documents/{memory_id}',
    headers={
        'Content-Type': 'application/json',
        'Authorization': 'Bearer SMARAN_API_KEY'
    },
