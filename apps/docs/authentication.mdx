...
All API requests require authentication using a Bearer token. Get your API key from the [Developer Platform](https://console.engram.ai).
...
const client = new Engram({ apiKey: "YOUR_API_KEY" });
```
...
When users connect external services (Google Drive, Notion, OneDrive), they see a "Log in to **Engram**" prompt by default. You can replace this with your own app name by providing your own OAuth credentials via the settings endpoint.
...
<Accordion title="Container-scoped keys" icon="lock">
  ...
  **Allowed endpoints:** `/v3/documents`, `/v3/memories`, `/v4/memories`, `/v3/search`, `/v4/search`, `/v4/profile`
  ...
  ```bash
  curl https://api.engram.ai/v3/auth/scoped-key \
    --request POST \
    --header 'Content-Type: application/json' \
    --header 'Authorization: Bearer YOUR_API_KEY' \
    -d '{
  ...
  ```bash
  curl https://api.engram.ai/v3/auth/scoped-key/KEY_ID \
    --request DELETE \
    --header 'Authorization: Bearer YOUR_API_KEY'
  ```
