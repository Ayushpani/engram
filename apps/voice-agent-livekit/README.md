...
This agent is wired to [Smaran](https://github.com/Ayushpani/smaran) for cross-session memory: facts
a user states in one call are recalled in a later, unrelated call — a plain
LLM's chat context is scoped to a single session and cannot do this.

- **Config**: set `SMARAN_URL` and `SMARAN_API_KEY` in `.env.local`. Leave
  `SMARAN_URL` empty to run with memory off (recall/save become no-ops, zero
  behavior change otherwise). `SMARAN_ENABLED=false` forces the same
  baseline explicitly, for A/B comparison.
- **How it's wired**: `Assistant.on_user_turn_completed` in `src/agent.py`
  recalls relevant memories before each reply and injects them as a system
  message, then saves the turn after. Text input (used by the eval scripts
  below) is routed through a custom `text_input_cb` in `room_options` —
  the SDK's default text path calls `generate_reply()` directly and never
  invokes `on_user_turn_completed`, so plugging in here is required for text
  clients to get the same memory behavior as voice.
- **Measured result**: see [`CROSS_SESSION_RESULTS.md`](./CROSS_SESSION_RESULTS.md) —
  100% cross-session recall with Smaran vs. ~0% (confidently wrong answers,
  not "I don't know") without it, across repeated trials.

To reproduce or extend the eval:

```bash
uv run python src/agent.py dev                                # terminal 1 — leave running
bash src/run_repeated_cross_session.sh <label> <N>             # terminal 2 — N repeated trials
```

Edit `src/session_a.json` (facts stated) / `src/session_b.json` (recall
questions + expected substrings) to change what's tested.
